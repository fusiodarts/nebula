...
{
  QListViewItemIteratorPrivate(uint f) :
    flags(f)
  {
    // nothing yet
  }
...
struct QListViewPrivate
{
  ...
  public:
    ...
    bool operator<(const SortableItem& i) const
    {
      return cmp(i) < 0;
    }
    bool operator<=(const SortableItem& i) const
  ...
  public:
    ItemColumnInfo() :
      pm(0), next(0), truncated(FALSE), dirty(FALSE), allow_rename(FALSE), width(0)
    {
    }
    ~ItemColumnInfo()
    {
  ...
  public:
    ViewColumnInfo() :
      align(Qt::AlignAuto), sortable(TRUE), next(0)
    {
    }
    ~ViewColumnInfo()
    {
...
QListViewItem::QListViewItem(QListView * parent, QString label1, QString label2, QString label3, QString label4,
...
QListViewItem::QListViewItem(QListViewItem * parent, QString label1, QString label2, QString label3, QString label4,
                             QString label5, QString label6, QString label7, QString label8)
{
  init();
  parent->insertItem(this);
...
QListViewItem::QListViewItem(QListView * parent, QListViewItem * after, QString label1, QString label2, QString label3,
...
QListViewItem::QListViewItem(QListViewItem * parent, QListViewItem * after, QString label1, QString label2,
...
void QListViewItem::startRename(int col)
{
  ...
  r.setWidth(QMIN(lv->header()->sectionSize(col) - 1, lv->contentsX() + lv->visibleWidth() - r.left()));
  if (col == 0)
...
void QListViewItem::sortChildItems(int column, bool ascending)
{
  ...
  QListViewPrivate::SortableItem * siblings = new QListViewPrivate::SortableItem[nChildren];
  ...
  if (lv) {
    ...
    if (mlenabled) {
      ...
      for (int c = 0; c < lv->columns(); ++c) {
        ...
        int tmph = lv->d->fontMetricsHeight + lv->fontMetrics().lineSpacing() * (lines - 1);
        h = QMAX(h, tmph);
      }
      h += 2 * lv->itemMargin();
    } else {
...
void QListViewItem::setPixmap(int column, const QPixmap & pm)
{
  ...
  if ((pm.isNull() && (!l->pm || l->pm->isNull())) || (l->pm && pm.serialNumber() == l->pm->serialNumber()))
    return;
  ...
  if (mlenabled && column == 0 && isOpen() && childCount()) {
    ...
    if (textheight < height()) {
      ...
      lv->style().drawComplexControl(QStyle::CC_ListView, p, lv,
                                     ...
                                     QStyle::SC_ListViewExpand, (uint) QStyle::SC_All, QStyleOption(this));
    }
  }
}
...
int QListViewItem::width(const QFontMetrics& fm, const QListView* lv, int c) const
{
  int w;
  if (mlenabled)
    w = fm.size(AlignVCenter, text(c)).width() + lv->itemMargin() * 2 - lv->d->minLeftBearing - lv->d->minRightBearing;
  else
    w = fm.width(text(c)) + lv->itemMargin() * 2 - lv->d->minLeftBearing - lv->d->minRightBearing;
...
void QListViewItem::paintFocus(QPainter *p, const QColorGroup &cg, const QRect & r)
{
  QListView *lv = listView();
  if (lv)
    lv->style().drawPrimitive(QStyle::PE_FocusRect, p, r, cg, (isSelected() ? QStyle::Style_FocusAtBorder
...
void QListViewItem::paintBranches(QPainter * p, const QColorGroup & cg, int w, int y, int h)
{
  ...
  if (!visible || !lv)
    return;
  lv->style().drawComplexControl(QStyle::CC_ListView, p, lv, QRect(0, y, w, h), cg,
                                 lv->isEnabled() ? QStyle::Style_Enabled : QStyle::Style_Default,
...
void QListView::drawContentsOffset(QPainter * p, int ox, int oy, int cx, int cy, int cw, int ch)
{
  ...
  if (!d->drawables || d->drawables->isEmpty() || d->topPixel > cy || d->bottomPixel < cy + ch - 1
  ...
  while ((current = it.current()) != 0) {
    ...
    if (ih > 0 && current->y < cy + ch && current->y + ih > cy) {
      ...
      bool drawActiveSelection = hasFocus() || d->inMenuMode
          || !style().styleHint(QStyle::SH_ItemView_ChangeHighlightOnFocus, this) || (currentItem()
          && currentItem()->renameBox && currentItem()->renameBox->hasFocus());
    ...
    if (current->i == d->focusItem && hasFocus() && d->allColumnsShowFocus) {
      p->save();
      int x = -contentsX();
      int w = header()->cellPos(header()->count() - 1) + header()->cellSize(header()->count() - 1);
      ...
      if (d->h->mapToActual(0) == 0 || (current->l == 0 && !rootIsDecorated())) {
        ...
        int xdepth = QMIN(treeStepSize() * (current->i->depth() + (rootIsDecorated() ? 1 : 0)) + itemMargin(),
            d->h->cellSize(cell));
    ...
    if (ih != ith && (current->i != d->r || d->rootIsExpandable) && current->y + ith > cy && current->y + ih < cy + ch
        && tx + current->l * treeStepSize() < cx + cw && tx + (current->l + 1) * treeStepSize() > cx) {
      ...
      r.setRect(crleft - ox, crtop - oy, crright - crleft, crbottom - crtop);

      if (r.isValid()) {
        p->save();
        p->translate(rleft - ox, crtop - oy);
        current->i->paintBranches(p, colorGroup(), treeStepSize(), rtop - crtop, r.height());
        p->restore();
      }
    }
  }
  ...
  if (d->r->totalHeight() < cy + ch)
    paintEmptyArea(p, QRect(cx - ox, d->r->totalHeight() - oy, cw, cy + ch - d->r->totalHeight()));

  int c = d->h->count() - 1;
  if (c >= 0 && d->h->cellPos(c) + d->h->cellSize(c) < cx + cw) {
...
void QListView::buildDrawableList() const
{
  ...
  while (!stack.isEmpty()) {
    ...
    if (cur->i->isOpen() && cur->i->childCount() && cur->y + ith > cy && cur->y + ih < cy + ch) {
      ...
      if (c && y < cy + ch)
        stack.push(new QListViewPrivate::Pending(cur->l + 1, y, c));
    }
...
void QListView::setColumnWidth(int column, int w)
{
  ...
  if (column < d->h->count() && oldw != w) {
    ...
    disconnect(d->h, SIGNAL(sizeChange(int,int,int)), this, SLOT(handleSizeChange(int,int,int)) );
    emit
...
void QListView::handleSizeChange(int section, int os, int ns)
{
  ...
  updateGeometries();
  bool fullRepaint = d->fullRepaintOnComlumnChange || sx != horizontalScrollBar()->value() || sv
  ...
  viewport()->repaint(left - 4 - d->ellipsisWidth, 0, 4 + d->ellipsisWidth, visibleHeight(), FALSE ); // border between the items and ellipses width
  ...
  if (align != AlignAuto && align != AlignLeft)
    viewport()->repaint(d->h->cellPos(actual) - contentsX(), 0, d->h->cellSize(actual), visibleHeight());
  ...
  if (currentItem() && currentItem()->renameBox) {
    ...
    if (currentItem()->renameCol == 0)
      r.setLeft(r.left() + itemMargin() + (currentItem()->depth() + (rootIsDecorated() ? 1 : 0)) * treeStepSize() - 1);
...
{
  if (o == d->h && e->type() >= QEvent::MouseButtonPress && e->type() <= QEvent::MouseMove) {
    QMouseEvent * me = (QMouseEvent *) e;
    QMouseEvent me2(me->type(), QPoint(me->pos().x(), me->pos().y() - d->h->height()), me->button(), me->state());
    switch (me2.type()) {
...
{
  if (!parentItem || parentItem->is_root)
    return 0;
  return parentItem;
}
...
void QListView::contentsMousePressEventEx(QMouseEvent * e)
{
  ...
  if (currentItem() && currentItem()->renameBox && !itemRect(currentItem()).contains(e->pos())) {
    d->startEdit = FALSE;
    if (d->defRenameAction == Reject)
      currentItem()->cancelRename(currentItem()->renameCol);
    else
  ...
  if ((i->isExpandable() || i->childCount()) && d->h->mapToLogical(d->h->cellAt(vp.x())) == 0) {
    ...
    if (it.current()) {
      ...
      QStyle::SubControl ctrl = style().querySubControl(QStyle::CC_ListView, this, QPoint(x1, e->pos().y()),
                                                        QStyleOption(i));
      if (ctrl == QStyle::SC_ListViewExpand && e->type()
          == style().styleHint(QStyle::SH_ListViewExpand_SelectMouseType, this)) {
        d->buttonDown = FALSE;
  ...
  {// calculate activatedP
    ...
    int xdepth = treeStepSize() * (i->depth() + (rootIsDecorated() ? 1 : 0)) + itemMargin();
  ...
  if (i && !d->buttonDown && vp.x() + contentsX() < itemMargin() + (i->depth() + (rootIsDecorated() ? 1 : 0))
      * treeStepSize())
    i = 0;
  d->pressedItem = i;
  ...
  mouseButtonPressed(e->button(), i, viewport()->mapToGlobal(vp), c);
...
void QListView::contentsMouseReleaseEventEx(QMouseEvent * e)
{
  ...
  if (d->selectionMode == Extended && d->focusItem == d->pressedItem && d->pressedSelected && d->focusItem
      && e->button() == LeftButton) {
    bool block = signalsBlocked();
    blockSignals(TRUE);
    clearSelection();
  ...
  if (i && i == d->pressedItem && (i->isExpandable() || i->childCount()) && !d->h->mapToLogical(d->h->cellAt(vp.x()))
      && e->button() == LeftButton && e->type() == style().styleHint(QStyle::SH_ListViewExpand_SelectMouseType, this)) {
    QPtrListIterator<QListViewPrivate::DrawableItem> it(*(d->drawables));
    while (it.current() && it.current()->i != i)
      ++it;
    if (it.current()) {
      int x1 = vp.x() + d->h->offset() - d->h->cellPos(d->h->mapToActual(0)) - (treeStepSize() * (it.current()->l - 1));
      QStyle::SubControl ctrl = style().querySubControl(QStyle::CC_ListView, this, QPoint(x1, e->pos().y()),
                                                        QStyleOption(i));
  ...
  if (i == d->pressedItem && i && i->isSelected() && e->button() == LeftButton && d->startEdit) {
    ...
    if (d->pressedColumn == 0)
      r.setLeft(r.left() + itemMargin() + (currentItem()->depth() + (rootIsDecorated() ? 1 : 0)) * treeStepSize() - 1);
    if (r.contains(e->pos()) && !(e->state() & (ShiftButton | ControlButton)))
  ...
  if (i && vp.x() + contentsX() < rootColumnPos + itemMargin() + (i->depth() + (rootIsDecorated() ? 1 : 0))
      * treeStepSize() && vp.x() + contentsX() > rootColumnPos) {
    i = 0;
  }
  ...
  if (emitClicked) {
    ...
    mouseButtonClicked(e->button(), i, viewport()->mapToGlobal(vp), i ? d->h->mapToLogical(d->h->cellAt(vp.x())) : -1);
    ...
    if (e->button() == RightButton) {
      if (!i) {
        if (!(e->state() & ControlButton))
          clearSelection();
        emit
        rightButtonClicked(0, viewport()->mapToGlobal(vp), -1);
        return;
      }
...
void QListView::contentsMouseMoveEvent(QMouseEvent * e)
{
  ...
  if (i && !i->isEnabled())
    return;
  if (i != d->highlighted && !(d->pressedItem && (d->pressedItem->isSelected() || d->selectionMode == NoSelection)
  ...
  if (!d->buttonDown || ((e->state() & LeftButton) != LeftButton && (e->state() & MidButton) != MidButton
      && (e->state() & RightButton) != RightButton))
    return;

  if (d->pressedItem && (d->pressedItem->isSelected() || d->selectionMode == NoSelection)
      && d->pressedItem->dragEnabled()) {
...
void QListView::focusOutEvent(QFocusEvent*)
{
  if (QFocusEvent::reason() == QFocusEvent::Popup && d->buttonDown)
    d->buttonDown = FALSE;
  if (style().styleHint(QStyle::SH_ItemView_ChangeHighlightOnFocus, this)) {
    d->inMenuMode = QFocusEvent::reason() == QFocusEvent::Popup || (qApp->focusWidget()
        && qApp->focusWidget()->inherits("QMenuBar"));
    if (!d->inMenuMode) {
      bool db = d->useDoubleBuffer;
      d->useDoubleBuffer = TRUE;
      viewport()->repaint(FALSE);
...
void QListView::keyPressEvent(QKeyEvent * e)
{
  ...
  switch (e->key()) {
    ...
    case Key_Return:
      d->currentPrefix.truncate(0);
      if (i && !i->isSelectable() && i->isEnabled() && (i->childCount() || i->isExpandable() || i->isOpen())) {
        i->setOpen(!i->isOpen());
        return;
      }
      e->ignore();
    ...
    case Key_Next:
      ...
      if (i2 == i || !r.isValid() || visibleHeight() <= itemRect(i).bottom()) {
        if (i2)
          i = i2;
  ...
  setCurrentItem(i);
  if (i->isSelectable())
    handleItemChange(old, wasNavigation && (e->state() & ShiftButton), wasNavigation && (e->state() & ControlButton));
...
QListViewItem * QListView::itemAt(const QPoint & viewPos) const
{
  ...
  while (c && c->i && (c->y + c->i->height() <= g || !c->i->isVisible() || c->i->parent()
      && !c->i->parent()->isVisible()))
    c = d->drawables->next();
...
void QListView::setSelectionMode(SelectionMode mode)
{
  ...
  if ((d->selectionMode == Multi || d->selectionMode == Extended) && (mode == QListView::Single || mode
...
{
  if (!item || item->isSelected() == selected || !item->isSelectable() || selectionMode() == NoSelection)
    return;
...
{
  if (d->selectionMode == Single || d->selectionMode == NoSelection)
    return;
...
struct QCheckListItemPrivate
{
  QCheckListItemPrivate() :
    exclusive(0), currentState(QCheckListItem::Off), statesDict(0), tristate(FALSE)
  {
  }
 ...
 */
QCheckListItem::QCheckListItem(QCheckListItem *parent, QListViewItem *after, const QString &text, Type tt) :
...
/*!
 ...
 RadioButtonController.
 */
QCheckListItem::QCheckListItem(QListViewItem *parent, QListViewItem *after, const QString &text, Type tt) :
  QListViewItem(parent, after, text)
{
  myType = tt;
  if (myType == RadioButton) {
...
/*!
 ...
 Radio buttons must be children of a \c RadioButtonController.
 */
QCheckListItem::QCheckListItem(QListView *parent, QListViewItem *after, const QString &text, Type tt) :
  QListViewItem(parent, after, text)
{
  myType = tt;
  if (tt == RadioButton)
...
{
  if (myType == RadioButton && d->exclusive && d->exclusive->d && d->exclusive->d->exclusive == this)
...
void QCheckListItem::setState(ToggleState s, bool update, bool store)
{
  ...
  if (myType == CheckBox) {
    setCurrentState(s);
    stateChange(state());
    if (update && parent() && parent()->rtti() == 1 && ((QCheckListItem*) parent())->type() == CheckBoxController)
    ...
    if (s == NoChange && childCount()) {
      ...
      int childCount = 0;
      while (item) {
        if (item->rtti() == 1 && (((QCheckListItem*) item)->type() == CheckBox || ((QCheckListItem*) item)->type()
            == CheckBoxController)) {
      ...
      if (update) {
        if (childCount > 0) {
          ...
          if (oldState != internalState() && parent() && parent()->rtti() == 1 && ((QCheckListItem*) parent())->type()
              == CheckBoxController)
            ((QCheckListItem*) parent())->updateController(update, store);
          ...
          setCurrentState(s);
          stateChange(state());
          if (parent() && parent()->rtti() == 1 && ((QCheckListItem*) parent())->type() == CheckBoxController)
            ((QCheckListItem*) parent())->updateController(update, store);
        }
      } else {
        setCurrentState(s);
...
void QCheckListItem::activate()
{
  ...
  if (activatedPos(pos)) {
    ...
    if (parent() && parent()->rtti() == 1 && ((QCheckListItem*) parent())->type() == RadioButtonController)
      parentControl = TRUE;
  ...
  if ((myType == CheckBox) || (myType == CheckBoxController)) {
    switch (internalState()) {
      ...
      break;
      case Off:
        if ((!isTristate() && myType == CheckBox) || (myType == CheckBoxController && !childCount())) {
          setState( On);
        } else {
          setState( NoChange);
  ...
  while (item && theState != NoChange) {
    if (item->rtti() == 1 && (((QCheckListItem*) item)->type() == CheckBox || ((QCheckListItem*) item)->type()
      ...
      if (first) {
        ...
        if (checkItem->internalState() == NoChange || theState != checkItem->internalState())
          theState = NoChange;
        else
          theState = checkItem->internalState();
      }
...
void QCheckListItem::setup()
{
  ...
  QListView *lv = listView();
  if (lv)
    h = QMAX(lv->style().pixelMetric(QStyle::PM_CheckListButtonSize, lv), h);
  h = QMAX(h, QApplication::globalStrut().height());
  setHeight(h);
}
 ...
 */
void QCheckListItem::paintCell(QPainter * p, const QColorGroup & cg, int column, int width, int align)
{
  if (!p)
    return;
  ...
  if (myType == RadioButtonController) {
    ...
    if (!parentControl)
      x += 3;
    if (!pixmap(0)) {
      lv->style().drawPrimitive(QStyle::PE_CheckListController, p, QRect(x, 0, boxsize, fm.height() + 2 + marg), cg,
                                styleflags, QStyleOption(this));
      r += boxsize + 4;
    }
  } else {
    Q_ASSERT( lv ); //###
    ...
    if ((myType == CheckBox) || (myType == CheckBoxController)) {
      lv->style().drawPrimitive(QStyle::PE_CheckListIndicator, p, QRect(x, y, boxsize, fm.height() + 2 + marg), cg,
                                styleflags, QStyleOption(this));
    } else { //radio button look
      lv->style().drawPrimitive(QStyle::PE_CheckListExclusiveIndicator, p,
                                QRect(x, y, boxsize, fm.height() + 2 + marg), cg, styleflags, QStyleOption(this));
    }
    r += boxsize + 4;
  }
...
void QCheckListItem::paintFocus(QPainter *p, const QColorGroup & cg, const QRect & r)
{
  ...
  if (parent() && parent()->rtti() == 1 && ((QCheckListItem*) parent())->type() == RadioButtonController)
    parentControl = TRUE;
  if (myType != RadioButtonController && intersect && (lv->rootIsDecorated() || myType == RadioButton || (myType
...
{
  if (!item || item->isOpen() == open || (open && !item->childCount() && !item->isExpandable()))
    return;
...
{
  if (parentItem && olderSibling && olderSibling->parentItem == parentItem && olderSibling != this) {
...
{
  return qt_iteratorprivate_dict ? qt_iteratorprivate_dict->find((void *) this) : 0;
}
...
void QListView::handleItemChange(QListViewItem *old, bool shift, bool control)
{
  if (d->selectionMode == Single) {
    ...
    if (shift) {
      selectRange(d->selectAnchor ? d->selectAnchor : old, d->focusItem, FALSE, TRUE,
                  (d->selectAnchor && !control) ? TRUE : FALSE );
    } else if (!control) {
      bool block = signalsBlocked();
      blockSignals(TRUE);
      selectAll(FALSE);
...
bool QListView::selectRange(QListViewItem *newItem, QListViewItem *oldItem, QListViewItem *anchorItem)
{
  ...
  int anchorPos = anchorItem ? anchorItem->itemPos() : 0, oldPos = oldItem ? oldItem->itemPos() : 0, newPos =
      newItem->itemPos();
  ...
  bool changed = FALSE;
  int topPos = top ? top->itemPos() : 0, bottomPos = bottom ? bottom->itemPos() : 0;
