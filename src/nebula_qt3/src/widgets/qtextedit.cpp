...
public:
  QTextEditPrivate() :
    preeditStart(-1), preeditLength(-1), ensureCursorVisibleInShowEvent(FALSE), tabChangesFocus(FALSE),
#ifndef QT_NO_CLIPBOARD
        clipboard_mode(QClipboard::Clipboard),
#endif
#ifdef QT_TEXTEDIT_OPTIMIZATION
        od(0), optimMode(FALSE), maxLogLines(-1), logOffset(0),
#endif
...
bool QRichTextDrag::decode(QMimeSource *e, QString &str, const QCString &mimetype, const QCString &subtype)
{
...
QTextEdit::QTextEdit(QWidget *parent, const char *name) :
  QScrollView(parent, name, WStaticContents | WNoAutoErase), doc(new QTextDocument(0)), undoRedoInfo(doc)
{
  init();
}
...
QTextEdit::QTextEdit(const QString& text, const QString& context, QWidget *parent, const char *name) :
  QScrollView(parent, name, WStaticContents | WNoAutoErase), doc(new QTextDocument(0)), undoRedoInfo(doc)
{
  init();
...
void QTextEdit::init()
{
  ...
  resizeContents(0, doc->lastParagraph() ? (doc->lastParagraph()->paragId() + 1)
      * doc->formatCollection()->defaultFormat()->height() : 0);
  ...
  connect(changeIntervalTimer, SIGNAL( timeout() ), this, SLOT( doChangeInterval() ) );
...
void QTextEdit::paintDocument(bool drawAll, QPainter *p, int cx, int cy, int cw, int ch)
{
  ...
  if ((hasSelectedText() && !style().styleHint(QStyle::SH_BlinkCursorWhenTextSelected)) || isReadOnly()
      || !cursorVisible || doc->hasSelection(QTextDocument::IMSelectionText))
    drawCur = FALSE;
  ...
  if (contentsY() < doc->y()) {
    p->fillRect(contentsX(), contentsY(), visibleWidth(), doc->y(), g.brush(backRole));
  }
  if (drawAll && doc->width() - contentsX() < cx + cw) {
    p->fillRect(doc->width() - contentsX(), cy, cx + cw - doc->width() + contentsX(), ch, g.brush(backRole));
  }
  ...
  if (contentsHeight() < visibleHeight() && (!doc->lastParagraph() || doc->lastParagraph()->isValid()) && drawAll)
    p->fillRect(0, contentsHeight(), visibleWidth(), visibleHeight() - contentsHeight(), g.brush(backRole));
}
...
void QTextEdit::keyPressEvent(QKeyEvent *e)
{
  ...
  switch (e->key()) {
    ...
    case Key_Delete:
      ...
      doKeyboardAction(e->state() & ControlButton ? ActionWordDelete : ActionDelete);
      clearUndoRedoInfo = FALSE;
    ...
    case Key_Backspace:
      ...
      doKeyboardAction(e->state() & ControlButton ? ActionWordBackspace : ActionBackspace);
      clearUndoRedoInfo = FALSE;
    break;
    ...
    break;
    default: {
      if (e->text().length() && (!(e->state() & ControlButton) &&
#ifndef Q_OS_MACX
          !(e->state() & AltButton) &&
#endif
          !(e->state() & MetaButton) || (((e->state() & ControlButton) | AltButton) == (ControlButton | AltButton)))
        ...
        if (e->key() == Key_Tab) {
          ...
          if (textFormat() == Qt::RichText && cursor->index() == 0 && (cursor->paragraph()->isListItem()
            ...
            undoRedoInfo.styleInformation = QTextStyleCommand::readStyleInformation(doc, undoRedoInfo.id,
                                                                                    undoRedoInfo.eid);
        ...
        if ((autoFormatting() & AutoBulletList) && textFormat() == Qt::RichText && cursor->index() == 0
            && !cursor->paragraph()->isListItem() && (e->text()[0] == '-' || e->text()[0] == '*')) {
          clearUndoRedo();
          undoRedoInfo.type = UndoRedoInfo::Style;
          undoRedoInfo.id = cursor->paragraph()->paragId();
          undoRedoInfo.eid = undoRedoInfo.id;
        ...
        switch (e->key()) {
          ...
          case Key_H:
            ...
            if (!cursor->paragraph()->prev() && cursor->atParagStart())
              break;
  ...
  emit
  cursorPositionChanged(cursor->paragraph()->paragId(), cursor->index());
...
void QTextEdit::doKeyboardAction(KeyboardAction action)
{
  ...
  switch (action) {
    ...
    case ActionBackspace:
      if (textFormat() == Qt::RichText && (cursor->paragraph()->isListItem() || cursor->paragraph()->listDepth())
          && cursor->index() == 0) {
        if (undoEnabled) {
          clearUndoRedo();
          undoRedoInfo.type = UndoRedoInfo::Style;
      ...
      if (action == ActionBackspace && !cursor->atParagStart()) {
        ...
        lastFormatted = cursor->paragraph();
      } else if (cursor->paragraph()->prev() || (action == ActionWordBackspace && !cursor->atParagStart())) {
        clearUndoRedo();
        doc->setSelectionStart(QTextDocument::Temp, *cursor);
...
void QTextEdit::viewportResizeEvent(QResizeEvent *e)
{
  ...
  if (e->oldSize().width() != e->size().width()) {
    bool stayAtBottom = e->oldSize().height() != e->size().height() && contentsY() > 0 && contentsY() >= doc->height()
        - e->oldSize().height();
    doResize();
    if (stayAtBottom)
      scrollToBottom();
...
{
  if (!isUpdatesEnabled() || !viewport()->isUpdatesEnabled() || !cursor->paragraph() || !cursor->paragraph()->isValid()
      || (!style().styleHint(QStyle::SH_BlinkCursorWhenTextSelected)
          && (d->optimMode ? optimHasSelection() : doc->hasSelection(QTextDocument::Standard, TRUE ))) || (visible
      && !hasFocus() && !viewport()->hasFocus() && !inDnD) || doc->hasSelection(QTextDocument::IMSelectionText)
      || isReadOnly())
    return;
  ...
  if (!cursor->nestedDepth()) {
    ...
    x = QMAX(x, 0);
    p.setClipRect(QRect(x - contentsX(), r.y() - cursor->totalOffsetY() + cursor->y() - contentsY(), 2 * dist, h));
    doc->drawParagraph(&p, cursor->paragraph(), x, r.y() - cursor->totalOffsetY() + cursor->y(), 2 * dist, h, pix, cg,
                       visible, cursor);
  } else {
    doc->drawParagraph(&p, cursor->paragraph(), r.x() - cursor->totalOffsetX(), r.y() - cursor->totalOffsetY(),
                       r.width(), r.height(), pix, cg, visible, cursor);
  }
...
void QTextEdit::contentsMousePressEvent(QMouseEvent *e)
{
  ...
  if (d->trippleClickTimer->isActive() && (e->globalPos() - d->trippleClickPoint).manhattanLength()
  ...
  if (e->button() == LeftButton) {
    ...
    if (isReadOnly() && linksEnabled()) {
      ...
      if (c.paragraph() && c.paragraph()->at(c.index()) && c.paragraph()->at(c.index())->isAnchor()) {
...
void QTextEdit::contentsMouseReleaseEvent(QMouseEvent * e)
{
  ...
  if (((!onLink.isEmpty() && onLink == pressedLink) || (!d->onName.isEmpty() && d->onName == d->pressedName))
    ...
    if (::qt_cast<QTextBrowser*>(this)) { // change for 4.0
      QConnectionList *clist = receivers("anchorClicked(const QString&,const QString&)");
  ...
  emit
  copyAvailable(doc->hasSelection(QTextDocument::Standard));
  emit selectionChanged();
}
...
void QTextEdit::contentsDropEvent(QDropEvent *e)
{
  ...
  if (QRichTextDrag::canDecode(e)) {
    ...
    if (hasSel && internalDrag) {
      ...
      if (((dropId > selStartId) || (dropId == selStartId && dropIndex > selStartIndex)) && ((dropId < selEndId)
          || (dropId == selEndId && dropIndex <= selEndIndex)))
        insertCursor = c1;
      if (dropId == selEndId && dropIndex > selEndIndex) {
        insertCursor = c1;
        if (selStartId == selEndId) {
          insertCursor.setIndex(dropIndex - (selEndIndex - selStartIndex));
        } else {
          insertCursor.setIndex(dropIndex - selEndIndex + selStartIndex);
        }
      }
...
void QTextEdit::handleMouseMove(const QPoint& pos)
{
  ...
  if (currentFormat && currentFormat->key() != cursor->paragraph()->at(cursor->index())->format()->key()) {
    ...
    currentFontChanged(currentFormat->font());
    emit
    currentColorChanged(currentFormat->color());
  ...
  if (currentAlignment != cursor->paragraph()->alignment()) {
    ...
    emit
    currentAlignmentChanged( currentAlignment);
...
void QTextEdit::updateMicroFocusHint()
{
  ...
  if (hasFocus() || viewport()->hasFocus()) {
    ...
    if (!readonly) {
      ...
      setMicroFocusHint(c.x() - contentsX() + frameWidth(), c.y() + cursor->paragraph()->rect().y() - contentsY()
...
void QTextEdit::formatMore()
{
  ...
  for (int i = 0; lastFormatted && (i < to || (firstVisible && lastTop < contentsY() + height())); i++) {
  ...
  if (bottom > contentsHeight()) {
    ...
    if (contentsHeight() < visibleHeight())
      updateContents(0, contentsHeight(), visibleWidth(), visibleHeight() - contentsHeight());
  }
...
void QTextEdit::insert(const QString &text, uint insertionFlags)
{
  ...
  lastFormatted = checkNewLine && cursor->paragraph()->prev() ? cursor->paragraph()->prev() : cursor->paragraph();
  QTextCursor oldCursor = *cursor;
...
bool QTextEdit::focusNextPrevChild(bool n)
{
  ...
  if (b) {
    ...
    int y = p->rect().y();
    while (p && len == 0 && p->at(start)->isCustom() && p->at(start)->customItem()->isNested()) {
...
void QTextEdit::setFormat(QTextFormat *f, int flags)
{
  ...
  if (currentFormat && currentFormat->key() != f->key()) {
    ...
    currentColorChanged(currentFormat->color());
    emit
    currentVerticalAlignmentChanged((VerticalAlignment) currentFormat->vAlign());
...
void QTextEdit::updateCurrentFormat()
{
  ...
  if (i > 0)
    --i;
  if (doc->useFormatCollection() && (!currentFormat || currentFormat->key()
      != cursor->paragraph()->at(i)->format()->key())) {
    if (currentFormat)
      currentFormat->removeRef();
...
#endif
  resetInputContext();
  if (!isModified() && isReadOnly() && this->context() == context && this->text() == text)
    return;
...
bool QTextEdit::find(const QString &expr, bool cs, bool wo, bool forward, int *para, int *index)
{
#ifdef QT_TEXTEDIT_OPTIMIZATION
  if (d->optimMode)
...
void QTextEdit::getSelection(int *paraFrom, int *indexFrom, int *paraTo, int *indexTo, int selNum) const
{
...
void QTextEdit::append(const QString &text)
{
  ...
  if (f == Qt::PlainText) {
    ...
    if (doc->useFormatCollection() && !doc->preProcessor() && currentFormat
        != cursor->paragraph()->at(cursor->index())->format()) {
...
bool QTextEdit::handleReadOnlyKeyEvent(QKeyEvent *e)
{
  switch (e->key()) {
    ...
    case Key_Enter:
    case Key_Space: {
      if (!doc->focusIndicator.href.isEmpty() || !doc->focusIndicator.name.isEmpty()) {
        ...
        if (!doc->focusIndicator.name.isEmpty()) {
          if (::qt_cast<QTextBrowser*>(this)) { // change for 4.0
            ...
            if (!signalsBlocked() && clist) {
              QUObject o[3];
              static_QUType_QString.set(o + 1, doc->focusIndicator.name);
              static_QUType_QString.set(o + 2, doc->focusIndicator.href);
              activate_signal(clist, o);
            }
          }
        }
...
void QTextEdit::scrollToAnchor(const QString& name)
{
  ...
  for (;;) {
    ...
    if (c->isAnchor()) {
      ...
      if (a == name || (a.contains('#') && QStringList::split('#', a).contains(name))) {
        setContentsPos(contentsX(), QMIN(cursor.paragraph()->rect().top() + cursor.totalOffsetY(), contentsHeight()
            - visibleHeight()));
        break;
      }
    }
        ...
        if (!doc->preProcessor()) {
          ...
          for (int i = 0; i < (int) txt.length(); ++i) {
            if (txt[i] != '\n' && oldC.paragraph()->at(oldC.index())->format()) {
              ...
              undoRedoInfo.d->text. setFormat(oldLen + i, oldC.paragraph()->at(oldC.index())->format(), TRUE );
            }
            oldC.gotoNextLetter();
          }
        }
...
{
  QCString st = pickSpecial(QApplication::clipboard()->data(d->clipboard_mode), TRUE, pt);
  if (!st.isEmpty())
    pasteSubType(st);
}
#endif
...
void QTextEdit::clearUndoRedo()
{
  ...
  undoRedoInfo.clear();
  emit
  undoAvailable(doc->commands()->isUndoAvailable());
...
bool QTextEdit::getParagraphFormat(int para, QFont *font, QColor *color, VerticalAlignment *verticalAlignment,
                                   int *alignment, QStyleSheetItem::DisplayMode *displayMode,
                                   QStyleSheetItem::ListStyle *listStyle, int *listDepth)
{
...
#ifdef QT_TEXTEDIT_OPTIMIZATION
  popup->setItemEnabled(d->id[IdCopy], d->optimMode ? optimHasSelection() : doc->hasSelection(QTextDocument::Standard,
                                                                                              TRUE ));
#else
...
void QTextEdit::updateCursor(const QPoint & pos)
{
  if (isReadOnly() && linksEnabled()) {
    ...
    if (c.paragraph() == doc->lastParagraph() && c.paragraph()->rect().y() + c.paragraph()->rect().height() < pos.y())
      insideParagRect = FALSE;
    if (insideParagRect && c.paragraph() && c.paragraph()->at(c.index()) && c.paragraph()->at(c.index())->isAnchor()) {
      if (!c.paragraph()->at(c.index())->anchorHref().isEmpty() && c.index() < c.paragraph()->length() - 1)
        onLink = c.paragraph()->at(c.index())->anchorHref();
      else
        onLink = QString::null;

      if (!c.paragraph()->at(c.index())->anchorName().isEmpty() && c.index() < c.paragraph()->length() - 1)
  ...
  for (i = 0; i < len; i++) {
    ...
    if (state == 1 && tagClose) {
      ...
      if (!tagStr.isEmpty()) {
        ...
        // to be a color tag.
        tag->type = format ? QTextEditOptimPrivate::Format : QTextEditOptimPrivate::Color;
        if (tagStr[0] == '/') {
          ...
          while (cur) {
            if (cur->leftTag) { // push right-tags encountered
              ...
              tmp = tagStack.pop();
              if (!tmp) {
                if ((("/" + cur->tag) == tag->tag) || (tag->tag == "/font" && cur->tag.left(4) == "font")) {
      ...
      if (startIndex != -1) {
        int l = (endIndex == -1) ? line->length() - startIndex : endIndex - startIndex;
...
/*! \internal
 ...
 color the string.
 */
void QTextEdit::optimSetTextFormat(QTextDocument * td, QTextCursor * cur, QTextFormat * f, int start, int end,
                                   QTextEditOptimPrivate::Tag * tag)
{
  int formatFlags = QTextFormat::Bold | QTextFormat::Italic | QTextFormat::Underline;
  cur->setIndex(start);
  ...
  if (!ssItem || tag->type == QTextEditOptimPrivate::Format) {
    ...
    if (tag->type == QTextEditOptimPrivate::Format) {
      ...
      // this format tag
      tag = tag->prev;
      while (tag && (tag->type == QTextEditOptimPrivate::Format || tag->leftTag)) {
...
/*! \internal */
void QTextEdit::optimDrawContents(QPainter * p, int clipx, int clipy, int clipw, int cliph)
{
  QFontMetrics fm(QScrollView::font());
  ...
  td->setFormat(QTextDocument::Standard, &f, QTextFormat::Color | QTextFormat::Font);
  ...
  if (d->od->tags) {
    ...
    for (; i < startLine + nLines; i++) {
      ...
      if ((cur.paragraph()->length() - 1 - lastIndex) > 0 && tmp && !tmp->leftTag) {
        optimSetTextFormat(td, &cur, &f, lastIndex, cur.paragraph()->length() - 1, tmp);
      }
  ...
  if (optimHasSelection()) {
    ...
    if (startLine <= selStart && endLine >= selEnd) {
      ...
      td->selectAll(QTextDocument::Standard);
    } else if (startLine > selStart && endLine >= selEnd && startLine <= selEnd) {
    ...
    } else if (startLine <= selStart && endLine < selEnd && endLine > selStart) {
...
void QTextEdit::optimMouseReleaseEvent(QMouseEvent * e)
{
  ...
  if (!inDoubleClick) {
    ...
    } else if (d->od->selStart.line == d->od->selEnd.line && d->od->selStart.index > d->od->selEnd.index) {
      qSwap(&d->od->selStart.index, &d->od->selEnd.index);
    }
    oldMousePos = e->pos();
    repaintContents(FALSE);
  ...
  inDoubleClick = FALSE;
  emit
  copyAvailable( optimHasSelection());
  emit selectionChanged();
}
...
/*! \internal */
void QTextEdit::optimSetSelection(int startLine, int startIdx, int endLine, int endIdx)
{
...
{
  if (d->od->selStart.line != d->od->selEnd.line || d->od->selStart.index != d->od->selEnd.index)
    return TRUE;
  return FALSE;
}
...
QString QTextEdit::optimSelectedText() const
{
  ...
  if (d->od->selStart.line == d->od->selEnd.line) {
    str = d->od->lines[LOGOFFSET(d->od->selEnd.line)].mid(d->od->selStart.index, d->od->selEnd.index
        - d->od->selStart.index);
  } else {
    int i = d->od->selStart.line;
    str = d->od->lines[LOGOFFSET(i)].right(d->od->lines[LOGOFFSET(i)].length() - d->od->selStart.index) + "\n";
    i++;
...
/*! \internal */
bool QTextEdit::optimFind(const QString & expr, bool cs, bool /*wo*/, bool fw, int * para, int * index)
{
  bool found = FALSE;
  int parag = para ? *para : d->od->search.line, idx = index ? *index : d->od->search.index, i;

  if (d->od->len == 0)
    return FALSE;

  for (i = parag; fw ? i < d->od->numLines : i >= 0; fw ? i++ : i--) {
    idx = fw ? d->od->lines[LOGOFFSET(i)].find(expr, idx, cs) : d->od->lines[LOGOFFSET(i)].findRev(expr, idx, cs);
    if (idx != -1) {
      found = TRUE;
      break;
    } else if (fw)
...
void QTextEdit::optimCheckLimit(const QString& str)
{
  if (d->maxLogLines > -1 && d->maxLogLines <= d->od->numLines) {
    ...
    while (t) {
      ...
      if (((uint) LOGOFFSET(t->line) < (uint) d->logOffset && (uint) LOGOFFSET(t->line)
          < (uint) LOGOFFSET(d->od->numLines) && (uint) LOGOFFSET(d->od->numLines) > (uint) d->logOffset)) {
        if (t->prev)
          t->prev->next = t->next;
        if (t->next)
          t->next->prev = t->prev;
