...
public:
  QDateTimeEditorPrivate() :
    frm(TRUE), parag(new QTextParagraph(0, 0, 0, FALSE )), focusSec(0)
  {
  ...
  int section(const QPoint& p)
  {
    ...
    for (uint i = 0; i < sections.count(); ++i) {
      if (idx >= sections[i].selectionStart() && idx <= sections[i].selectionEnd())
        return i;
    }
    return -1;
  }
  ...
  void paint(const QString& txt, bool focus, QPainter& p, const QColorGroup& cg, const QRect& rect, QStyle& style)
  {
    int fw = 0;
    if (frm)
    ...
    QTextFormat *fb = parag->formatCollection()->format(p.font(), cg.base());
    QTextFormat *nf = parag->formatCollection()->format(p.font(), cg.text());
...
protected:
  void applyFocusSelection()
  {
    if (focusSec > -1) {
      ...
      parag->format();
      if (parag->at(selstart)->x < offset || parag->at(selend)->x + parag->string()->width(selend) > offset
          + sz.width()) {
...
/*!
 ...
 called \a name.
 */
QDateTimeEditor::QDateTimeEditor(QDateTimeEditBase * widget, QWidget *parent, const char * name) :
...
void QDateTimeEditor::paintEvent(QPaintEvent *)
{
  ...
  const QBrush &bg = colorGroup().brush(isEnabled() ? QColorGroup::Base : QColorGroup::Background);
  buffer.painter()->fillRect(0, 0, width(), height(), bg);
  d->paint(txt, hasFocus(), *buffer.painter(), colorGroup(), rect(), style());
  buffer.end();
}
    ...
    if (e->type() == QEvent::KeyPress) {
      ...
      switch (ke->key()) {
        ...
        case Key_BackTab: {
          ...
          while (w) {
            if (::qt_cast<QDateTimeSpinWidget*>(w) && qstrcmp(w->name(), "qt_spin_widget") != 0 || ::qt_cast<
                QDateTimeEdit*>(w))
              break;
...
void QDateEdit::init()
{
  ...
  d->controls = new QDateTimeSpinWidget(this, qstrcmp(name(), "qt_datetime_dateedit") == 0 ? "qt_spin_widget"
                                                                                           : "date edit controls");
  ...
  connect(this, SIGNAL( valueChanged(const QDate&) ), SLOT( updateButtons() ) );
...
QString QDateEdit::sectionFormattedText(int sec)
{
  ...
  if (d->typing && sec == d->ed->focusSection())
    d->ed->setSectionSelection(sec, sectionOffsetEnd(sec) - txt.length(), sectionOffsetEnd(sec));
  else
    d->ed->setSectionSelection(sec, sectionOffsetEnd(sec) - sectionLength(sec), sectionOffsetEnd(sec));
...
void QTimeEdit::init()
{
  ...
  d->controls = new QDateTimeSpinWidget(this, qstrcmp(name(), "qt_datetime_timeedit") == 0 ? "qt_spin_widget"
                                                                                           : "time edit controls");
...
bool QTimeEdit::outOfRange(int h, int m, int s) const
{
  if (QTime::isValid(h, m, s)) {
    ...
    if (currentTime > maxValue() || currentTime < minValue())
      return TRUE;
    else
      return FALSE;
  }
...
void QTimeEdit::addNumber(int sec, int num)
{
  ...
  switch (sec) {
    case 0:
      txt = (d->display & AMPM && d->h > 12) ? QString::number(d->h - 12) : QString::number(d->h);
...
QSize QTimeEdit::sizeHint() const
{
  ...
  int w = 2 + fm.width('9') * 6 + fm.width(d->ed->separator()) * 2 + d->controls->upRect().width() + fw * 4;
  if (d->display & AMPM) {
    if (lAM)
      w += fm.width(*lAM) + 4;
    else
...
/*!
 ...
 parent \a parent and called \a name.
 */
QDateTimeEdit::QDateTimeEdit(const QDateTime& datetime, QWidget * parent, const char * name) :
  QWidget(parent, name)
{
  init();
...
QSize QDateTimeEdit::minimumSizeHint() const
{
  ...
  return QSize(dsh.width() + tsh.width(), QMAX(dsh.height(), tsh.height()));
}
...
void QDateTimeEdit::init()
{
  ...
  d->adv = FALSE;
  connect(de, SIGNAL( valueChanged(const QDate&) ), this, SLOT( newValue(const QDate&) ) );
  connect(te, SIGNAL( valueChanged(const QTime&) ), this, SLOT( newValue(const QTime&) ) );
  setFocusProxy( de);
  setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Fixed);
}
