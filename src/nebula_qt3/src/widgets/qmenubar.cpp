...
void QMenuBar::menuInsPopup(QPopupMenu *popup)
{
  ...
  connect(popup, SIGNAL(highlightedRedirect(int)), SLOT(subHighlighted(int)) );
  connect(popup, SIGNAL(destroyed(QObject*)), this, SLOT(popupDestroyed(QObject*)) );
}
...
void QMenuBar::menuDelPopup(QPopupMenu *popup)
{
  ...
  disconnect(popup, SIGNAL(destroyed(QObject*)), this, SLOT(popupDestroyed(QObject*)) );
}
...
bool QMenuBar::eventFilter(QObject *object, QEvent *event)
{
  ...
  if (object == this && event->type() == QEvent::LanguageChange) {
    ...
    return FALSE;
  } else if (!(event->type() == QEvent::Accel || event->type() == QEvent::AccelOverride || event->type()
      == QEvent::KeyPress || event->type() == QEvent::KeyRelease)
  ...
  if (((QWidget*) object)->focusWidget() == object
    ...
    if (waitforalt && event->type() == QEvent::KeyRelease && (ke->key() == Key_Alt || ke->key() == Key_Meta)
#if defined(Q_WS_X11)
        && QMenuData::d->aInt == qt_xfocusout_grab_counter
#endif
    ) {
    ...
    } else if (!hasFocus() && (event->type() == QEvent::AccelOverride) && !(((QKeyEvent *) event)->key() == Key_Alt
...
bool QMenuBar::tryMouseEvent(QPopupMenu *, QMouseEvent *e)
{
  ...
  if (item == -1 && (e->type() == QEvent::MouseButtonPress || e->type() == QEvent::MouseButtonRelease)) {
    hidePopups();
    goodbye();
    return FALSE;
  }
...
int QMenuBar::calculateRects(int max_width)
{
  ...
  while (i < (int) mitems->count()) { // for each menu item...
    ...
    if (!mi->isSeparator() || mi->widget()) {
      ...
      if (((!reverse && x + w + frameWidth() - max_width > 0) || (reverse && x - w - itemSpacing - frameWidth() < 0))
          && nlitems > 0) {
        nlitems = 0;
        x = frameWidth();
        y += h;
...
void QMenuBar::drawContents(QPainter *p)
{
  ...
  for (int i = 0; i < (int) mitems->count(); i++) {
    ...
    if (!mi->text().isNull() || mi->pixmap()) {
      ...
      style().drawControl(QStyle::CE_MenuBarItem, buffer.painter(), this, r, g, flags, QStyleOption(mi));
    }
  }
...
void QMenuBar::mouseReleaseEvent(QMouseEvent *e)
{
  ...
  int item = itemAtPos(e->pos());
  if (item >= 0 && !mitems->at(item)->isEnabledAndVisible() || actItem >= 0
      && !mitems->at(actItem)->isEnabledAndVisible()) {
    hidePopups();
    setActiveItem(-1);
    return;
  }
  ...
  // pressing an item twice closes in windows, but not in motif :/
      style().styleHint(QStyle::SH_GUIStyle) == WindowsStyle && actItem == item) {
    showMenu = FALSE;
    setAltMode(FALSE);
  }
...
void QMenuBar::keyPressEvent(QKeyEvent *e)
{
  ...
  if (e->state() & Qt::ControlButton && (e->key() == Qt::Key_Tab || e->key() == Qt::Key_Backtab)) {
    e->ignore();
    return;
  }
    ...
    if (!s.isEmpty()) {
      ...
      if (i) {
        if (!autoaccel) {
          ...
          connect(autoaccel, SIGNAL(activated(int)), SLOT(accelActivated(int)) );
          connect(autoaccel, SIGNAL(activatedAmbiguously(int)), SLOT(accelActivated(int)) );
          connect(autoaccel, SIGNAL(destroyed()), SLOT(accelDestroyed()) );
        }
