/****************************************************************************
** $Id: qt/qdir_unix.cpp   3.3.8   edited Jan 11 14:38 $
**
...
extern bool aq_access_restricted(QFileInfo *);
#endif
                          ...
                          int filterSpec, int sortSpec)
{
...
#endif // QT_THREAD_SUPPORT && _POSIX_THREAD_SAFE_FUNCTIONS
  {
    ...
    if ((doDirs && fi.isDir()) || (doFiles && fi.isFile()) ||
      ...
      // ### AbanQ
#ifdef AQ_FILE_ACCESS_RESTRICTED
      QFileInfo *fiInf = new QFileInfo(fi);
      if (aq_access_restricted(fiInf))
        continue;
      fiList->append(fiInf);
#else
...
bool QDir::mkpath(const QString &dirPath) const
{
  ...
  dirName = QDir::convertSeparators(dirName);
#endif
  for (int oldslash = -1, slash = 0; slash != -1; oldslash = slash) {
    ...
    if (slash) {
      QCString chunk = QFile::encodeName(dirName.left(slash));
      QT_STATBUF st;
...
bool QDir::rmpath(const QString &dirPath) const
{
  ...
  for (int oldslash = 0, slash = dirName.length(); slash > 0; oldslash = slash) {
    ...
    slash = dirName.findRev(QDir::separator(), oldslash - 1);
  }
  return true;
}
// ### AbanQ
