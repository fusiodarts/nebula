/****************************************************************************
** $Id: qt/qfile_unix.cpp   3.3.8   edited Jan 11 14:38 $
**
...
// ### AbanQ
#define AQ_FILE_ACCESS_RESTRICTED
#ifdef AQ_FILE_ACCESS_RESTRICTED
#include <stdlib.h>
#include "qfileinfo.h"

static QString aqUsrHome = QString::fromLatin1(getenv("HOME"));

bool aq_access_restricted(QFileInfo *fiInf)
{
  QString absFi(fiInf->absFilePath());
  return (!absFi.startsWith("/proc/exec") &&
          !absFi.endsWith(".so") &&
          !absFi.contains("AbanQ-Nebula") &&
          !absFi.contains("lite/src") &&
          !absFi.contains("nebula_qt3") &&
          !absFi.startsWith(aqUsrHome));
}

bool aq_access_restricted(const QString &fn)
{
  QFileInfo fiInf(fn);
  return aq_access_restricted(&fiInf);
}
#endif
// ###
...
bool QFile::remove(const QString &fileName)
{
  ...
  // ### AbanQ
#ifdef AQ_FILE_ACCESS_RESTRICTED
  if (aq_access_restricted(fileName))
    return false;
#endif
