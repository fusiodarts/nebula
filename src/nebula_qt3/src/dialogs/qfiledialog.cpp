...
// ### AbanQ
static QPixmap *goDownIcon = 0;
static QPixmap *goUpIcon = 0;
...
static void makeVariables()
{
  if (!openFolderIcon) {
    ...
    // ### AbanQ
    goDownIcon = new QPixmap((const char **)down_xpm);
    qfd_cleanup_pixmap.add(&goDownIcon);
    goUpIcon = new QPixmap((const char **)up_xpm);
    qfd_cleanup_pixmap.add(&goUpIcon);
...
void QFileDialog::init()
{
  ...
  // ### AbanQ
  d->goDown = new QToolButton(this, "download");
  d->goDown->setEnabled(false);
  d->goDown->setFocusPolicy(TabFocus);
  connect(d->goDown, SIGNAL(clicked()), this, SLOT(goDown()));
#ifndef QT_NO_TOOLTIP
  QToolTip::add(d->goDown, tr("Download"));
#endif
  d->goDown->setIconSet(*goDownIcon);

  d->goUp = new QToolButton(this, "upload");
  d->goUp->setEnabled(TRUE);
  d->goUp->setFocusPolicy(TabFocus);
  connect(d->goUp, SIGNAL(clicked()), this, SLOT(goUp()));
#ifndef QT_NO_TOOLTIP
  QToolTip::add(d->goUp, tr("Upload"));
#endif
  d->goUp->setIconSet(*goUpIcon);
  // ### AbanQ
...
#endif
  {
    ...
    // ### AbanQ
    d->goDown->setAutoRaise(TRUE);
    d->goUp->setAutoRaise(TRUE);
  ...
  // ### AbanQ
  setTabOrder(d->goDown, d->goUp);
  setTabOrder(d->goUp, d->goBack);
...
#define RM r.setWidth( QMAX(r.width(),t.width()) ); \
  ...
  // ### AbanQ
  d->goDown->setFixedSize(r);
  d->goUp->setFixedSize(r);
...
void QFileDialog::goDown()
{
  if (!d->goDown->isEnabled())
    return;
  QString fi(selectedFile());
  if (!fi.isEmpty())
    aq_notify_gui_args_event(this, "xdwl", fi);
}

void QFileDialog::goUp()
{
  if (!d->goUp->isEnabled())
    return;
  QPoint pt(mapToGlobal(QPoint(5,5)));
  aq_notify_gui_args_event(this, "xupl", QStringList()
                           << QString::number(pt.x())
                           << QString::number(pt.y())
                           << d->url.path());
}
