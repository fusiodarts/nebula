...
static void create_dpis()
{
  ...
  for (i = 0; i < screens; i++) {
    ...
    dpisY[i] = (DisplayHeight(dpy, i) * 254 + DisplayHeightMM(dpy, i) * 5) / (DisplayHeightMM(dpy, i) * 10);
  }
}
...
// ### AbanQ
extern void aq_bitblt(QWidget*, int, int, const QPaintDevice*, int, int, int, int);

void bitBlt(QPaintDevice *dst, int dx, int dy, const QPaintDevice *src, int sx, int sy, int sw, int sh,
            Qt::RasterOp rop, bool ignoreMask)
{
  ...
  static const short ropCodes[] = { // ROP translation table
      GXcopy, GXor, GXxor, GXandInverted, GXcopyInverted, GXorInverted, GXequiv, GXand, GXinvert, GXclear, GXset,
          GXnoop, GXandReverse, GXorReverse, GXnand, GXnor };
  if (rop > Qt::LastROP) {
#if defined(QT_CHECK_RANGE)
  ...
  if (td == QInternal::Pixmap) {
    ...
    mono_dst = FALSE;
    include_inferiors = include_inferiors || ((QWidget*) dst)->testWFlags(Qt::WPaintUnclipped);
  }
  ...
  if (src_pm && !mono_src && src_pm->data->alphapm && !ignoreMask) {
    ...
    if (src->x11RenderHandle() && alpha->x11RenderHandle() && dst->x11RenderHandle()) {
      ...
      XRenderComposite(dpy, PictOpOver, src->x11RenderHandle(), alpha->x11RenderHandle(), dst->x11RenderHandle(), sx,
                       sy, sx, sy, dx, dy, sw, sh);
      // restore attributes
      ...
      // ### AbanQ
      if (td == QInternal::Widget)
        aq_bitblt((QWidget*) dst, dx, dy, src, sx, sy, sw, sh);
  ...
  if (mask && !mono_src) { // fast masked blt
    ...
    if (mask->data->maskgc) {
      ...
      if (FALSE && src_pm->optimization() == QPixmap::NormalOptim) { // #### cache disabled
        ...
        gc = cache_mask_gc(dpy, dst->handle(), mask->data->ser_no, mask->handle());
      } else {
  ...
  if (mono_src && mono_dst && src == dst) { // dst and src are the same bitmap
    ...
    XGCValues gcvals;
    ulong valmask = GCBackground | GCForeground | GCFillStyle | GCStipple | GCTileStipXOrigin | GCTileStipYOrigin;
