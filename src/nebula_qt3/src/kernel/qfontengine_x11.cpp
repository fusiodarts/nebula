...
extern bool qt_xForm_helper(const QWMatrix &trueMat, int xoffset, int type, int depth, uchar *dptr, int dbpl,
                            int p_inc, int dHeight, uchar *sptr, int sbpl, int sWidth, int sHeight);
...
static void drawScaled(int x, int y, const QTextEngine *engine, const QScriptItem *si, int textFlags, Display *dpy,
                       GC gc, QPaintDevice *pdev, QFontEngine *fe, const QWMatrix &xmat, float scale)
{
  ...
  if (br2.width() <= 0 || br2.height() <= 0 || br2.width() >= 32768 || br2.height() >= 32768)
    return;
...
QFontEngine::Error QFontEngineBox::stringToCMap(const QChar *, int len, glyph_t *glyphs, advance_t *advances,
                                                int *nglyphs, bool) const
{
  if (*nglyphs < len) {
    *nglyphs = len;
    return OutOfMemory;
...
static inline XCharStruct *charStruct(XFontStruct *xfs, uint ch)
{
  ...
  if (r >= xfs->min_byte1 && r <= xfs->max_byte1 && c >= xfs->min_char_or_byte2 && c <= xfs->max_char_or_byte2) {
    if (!xfs->per_char)
      xcs = &(xfs->min_bounds);
    else {
      xcs = xfs->per_char + ((r - xfs->min_byte1) * (xfs->max_char_or_byte2 - xfs->min_char_or_byte2 + 1)) + (c
  ...
  _fs(fs), _name(name), _codec(0), _scale(1.), _cmap(mib)
{
  ...
  cache_cost = (((fs->max_byte1 - fs->min_byte1) * (fs->max_char_or_byte2 - fs->min_char_or_byte2 + 1))
      + fs->max_char_or_byte2 - fs->min_char_or_byte2);
  cache_cost = ((fs->max_bounds.ascent + fs->max_bounds.descent) * (fs->max_bounds.width * cache_cost / 8));
  lbearing = SHRT_MIN;
  rbearing = SHRT_MIN;
...
QFontEngine::Error QFontEngineXLFD::stringToCMap(const QChar *str, int len, glyph_t *glyphs, advance_t *advances,
                                                 int *nglyphs, bool mirrored) const
{
  ...
  if (_codec) {
    ...
    if (haveNbsp || mirrored) {
      ...
      for (int i = 0; i < len; i++)
        chars[i] = (str[i].unicode() == 0xa0 ? 0x20 : (mirrored ? ::mirroredChar(str[i]).unicode() : str[i].unicode()));
    }
...
void QFontEngineXLFD::draw(QPainter *p, int x, int y, const QTextEngine *engine, const QScriptItem *si, int textFlags)
{
  ...
  if (p->txop > QPainter::TxTranslate || _scale < 0.9999 || _scale > 1.0001) {
    ...
    if (!degenerate && xlfd_transformations != XlfdTrUnsupported) {
      ...
      int i = 0;
      while (trf) {
        if (trf->xx == mat[0] && trf->xy == mat[1] && trf->yx == mat[2] && trf->yy == mat[3])
          break;
        TransformedFont *tmp = trf;
        trf = trf->next;
        if (i > 10) {
...
glyph_metrics_t QFontEngineXLFD::boundingBox(const glyph_t *glyphs, const advance_t *advances,
...
{
  int l = qRound((QMIN(_fs->ascent, _fs->max_bounds.ascent) + QMIN(_fs->descent, _fs->max_bounds.descent)) * _scale
      * 0.15);
  return (l > 0) ? l : 1;
}
...
{
  return (((f->max_byte1 - f->min_byte1) * (f->max_char_or_byte2 - f->min_char_or_byte2 + 1)) + f->max_char_or_byte2
      - f->min_char_or_byte2);
}
  ...
  if (lbearing == SHRT_MIN) {
    if (_fs->per_char) {
      ...
      for (int c = 1; c < nc; c++) {
        ...
        if ((cs[c].lbearing <= 0 && cs[c].rbearing <= 0) || (cs[c].lbearing >= cs[c].width && cs[c].rbearing
            >= cs[c].width))
          continue;
...
QFontEngineLatinXLFD::QFontEngineLatinXLFD(XFontStruct *xfs, const char *name, int mib)
{
...
void QFontEngineLatinXLFD::findEngine(const QChar &ch)
{
  ...
  static const char *alternate_encodings[] = { "iso8859-1", "iso8859-2", "iso8859-3", "iso8859-4", "iso8859-9",
                                               "iso8859-10", "iso8859-13", "iso8859-14", "iso8859-15", "hp-roman8" };
  ...
  if (!(_count % engine_array_inc)) {
    ...
    int new_size = (((_count + engine_array_inc) / engine_array_inc) * engine_array_inc);
  ...
  for (i = 0; i < 0x200; ++i) {
    if (glyphIndices[i] != 0 || glyphs[i] == 0)
      continue;
...
QFontEngine::Error QFontEngineLatinXLFD::stringToCMap(const QChar *str, int len, glyph_t *glyphs, advance_t *advances,
                                                      int *nglyphs, bool mirrored) const
{
  if (*nglyphs < len) {
    *nglyphs = len;
  ...
  if (advances) {
    ...
    if (mirrored) {
      ...
      while (c != str) {
        --c;
        --g;
        *g = ((c->unicode() < 0x200) ? glyphIndices[c->unicode()] : (c->unicode() == 0x20ac) ? euroIndex : 0);
        missing = (missing || (*g == 0));
      }
    }
  }
...
void QFontEngineLatinXLFD::draw(QPainter *p, int x, int y, const QTextEngine *engine, const QScriptItem *si,
                                int textFlags)
{
  if (!si->num_glyphs)
    return;
...
glyph_metrics_t QFontEngineLatinXLFD::boundingBox(const glyph_t *glyphs_const, const advance_t *advances,
                                                  const qoffset_t *offsets, int numGlyphs)
{
  if (numGlyphs <= 0)
  ...
  for (end = 0; end < numGlyphs; ++end) {
    ...
    const glyph_metrics_t gm = _engines[which]->boundingBox(glyphs + start, advances + start, offsets + start, end
        - start);

    overall.x = QMIN(overall.x, gm.x);
    overall.y = QMIN(overall.y, gm.y);
    overall.width = overall.xoff + gm.width;
    overall.height = QMAX(overall.height + overall.y, gm.height + gm.y) - QMIN(overall.y, gm.y);
...
bool QFontEngineLatinXLFD::canRender(const QChar *string, int len)
{
  ...
  int i;
  for (i = 0; i < len; ++i) {
    if (string[i].unicode() >= 0x200 || glyphIndices[string[i].unicode()] == 0) {
...
void QFontEngineLatinXLFD::setScale(double scale)
{
  ...
  _engines[0]->stringToCMap((const QChar *) chars, 0x200, glyphIndices, glyphAdvances, &glyphCount, FALSE );
}
...
#endif // QT_XFT2
  cache_cost = _font->height * _font->max_advance_width * (_face ? _face->num_glyphs : 1024);

  // if the Xft font is not antialiased, it uses bitmaps instead of
  // 8-bit alpha maps... adjust the cache_cost to reflect this
  Bool antialiased = TRUE;
  if (XftPatternGetBool(pattern, XFT_ANTIALIAS, 0, &antialiased) == XftResultMatch && !antialiased) {
    cache_cost /= 8;
  }
  lbearing = SHRT_MIN;
  rbearing = SHRT_MIN;
...
QFontEngine::Error QFontEngineXft::stringToCMap(const QChar *str, int len, glyph_t *glyphs, advance_t *advances,
                                                int *nglyphs, bool mirrored) const
{
  if (*nglyphs < len) {
    *nglyphs = len;
...
void QFontEngineXft::draw(QPainter *p, int x, int y, const QTextEngine *engine, const QScriptItem *si, int textFlags)
{
  ...
  if (p->txop >= QPainter::TxScale || p->rop != Qt::CopyROP || _scale < 0.9999 || _scale > 1.001) {
    ...
    int i = 0;
    while (trf) {
      if (trf->xx == (float) m2.xx && trf->xy == (float) m2.xy && trf->yx == (float) m2.yx && trf->yy == (float) m2.yy)
        break;
      TransformedFont *tmp = trf;
      trf = trf->next;
      if (i > 10) {
...
glyph_metrics_t QFontEngineXft::boundingBox(const glyph_t *glyphs, const advance_t *advances, const qoffset_t *offsets,
                                            int numGlyphs)
{
  XGlyphInfo xgi;
...
{
  int l = qRound(QMIN(_font->height - (_font->ascent + _font->descent), ((_font->ascent + _font->descent) >> 4)
      * _scale));
  return (l > 0) ? l : 1;
}
...
static const ushort char_table[] = { 40, 67, 70, 75, 86, 88, 89, 91, 102, 114, 124, 127, 205, 645, 884, 922, 1070,
                                     12386 };
  ...
  if (gpos) {
    ...
    for (unsigned int i = 0; i < otl_buffer->in_length; i++) {
      ...
      if (positions[i].new_advance) {
        item->advances[i] = item->flags & QTextEngine::RightToLeft ? -qRound((positions[i].x_advance >> 6) * scale)
