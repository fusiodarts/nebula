...
void QPixmap::init(int w, int h, int d, bool bitmap, Optimization optim)
{
  ...
  data->w = w;
  data->h = h;
  hd = (HANDLE) XCreatePixmap(x11Display(), RootWindow(x11Display(), x11Screen()), w, h, data->d);
  ...
  if (qt_has_xft) {
    if (data->d == 1) {
      ...
      rendhd = (HANDLE) XftDrawCreate(x11Display(), hd, (Visual *) x11Visual(), x11Colormap());
    }
  }
#endif // QT_NO_XFTFREETYPE
}
...
{ // for bitmaps only
  ...
  hd = (HANDLE) XCreateBitmapFromData(x11Display(), RootWindow(x11Display(), x11Screen()), (char *) bits, w, h);
  ...
  if (m == QPaintDeviceMetrics::PdmWidth)
    ...
    switch (m) {
      ...
      case QPaintDeviceMetrics::PdmWidthMM:
        val = (DisplayWidthMM(dpy, scr) * width()) / DisplayWidth(dpy, scr);
      break;
      case QPaintDeviceMetrics::PdmHeightMM:
        val = (DisplayHeightMM(dpy, scr) * height()) / DisplayHeight(dpy, scr);
      break;
...
QImage QPixmap::convertToImage() const
{
  ...
  if (!xi) // fetch data from X server
    xi = XGetImage(x11Display(), hd, 0, 0, w, h, AllPlanes, mono ? XYPixmap : ZPixmap);
  Q_CHECK_PTR( xi );
  if (!xi)
    return image; // null image
    ...
    for (int y = 0; y < h; y++) {
      ...
      for (int x = 0; x < w; x++) {
        switch (bppc) {
          ...
          break;
          case 24: // 24 bit MSB
            pixel = src[2] | (ushort) src[1] << 8 | (uint) src[0] << 16;
            src += 3;
          break;
          case 25: // 24 bit LSB
            pixel = src[0] | (ushort) src[1] << 8 | (uint) src[2] << 16;
            src += 3;
          break;
          case 32: // 32 bit MSB
            pixel = src[3] | (ushort) src[2] << 8 | (uint) src[1] << 16 | (uint) src[0] << 24;
            src += 4;
          break;
          case 33: // 32 bit LSB
            pixel = src[0] | (ushort) src[1] << 8 | (uint) src[2] << 16 | (uint) src[3] << 24;
            src += 4;
          break;
        ...
        if (alf) {
          *dst++ = qRgba(r, g, b, asrc[x]);
          ...
          if (ale) {
            *dst++ = (asrc[x >> 3] & (1 << (x & 7))) ? qRgba(r, g, b, 0xff) : qRgba(r, g, b, 0x00);
          } else {
            *dst++ = (asrc[x >> 3] & (1 << (7 - (x & 7)))) ? qRgba(r, g, b, 0xff) : qRgba(r, g, b, 0x00);
          }
        } else {
          *dst++ = qRgb(r, g, b);
        }
...
#if defined(QT_CHECK_RANGE)
    qWarning("QPixmap::convertToImage: Display not supported (bpp=%d)", xi->bits_per_pixel);
#endif
    image.reset();
    return image;
  }
  ...
  if (mono) { // bitmap
    ...
    for (i = 0; i < 256; i++) { // translate pixels
      if (use[i]) {
        image.setColor(j++, (msk ? 0xff000000 : 0) | qRgb((carr[i].red >> 8) & 255, (carr[i].green >> 8) & 255,
                                                          (carr[i].blue >> 8) & 255));
      }
    }
  ...
  if (data->optim != BestOptim) { // throw away image data
    ...
    // ### AbanQ
    ((QPixmap*) this)->data->hashKey_ = 0;
  } else {
    // ### AbanQ
    if (((QPixmap*) this)->data->ximage != xi)
      ((QPixmap*) this)->data->hashKey_ = 0;
...
bool QPixmap::convertFromImage(const QImage &img, int conversion_flags)
{
  ...
  if (d == 1) { // 1 bit pixmap (bitmap)
    ...
    hd = (HANDLE) XCreateBitmapFromData(x11Display(), RootWindow(x11Display(), x11Screen()), bits, w, h);
  ...
  if (trucol) { // truecolor display
    ...
    if (d8) { // setup pixel translation
      ...
      for (int i = 0; i < image.numColors(); i++) {
        ...
        pix[i] = (b & blue_mask) | (g & green_mask) | (r & red_mask) | ~(blue_mask | green_mask | red_mask);
      }
    }
    ...
    bool contig_bits = n_bits(red_mask) == rbits && n_bits(green_mask) == gbits && n_bits(blue_mask) == bbits;
    bool dither_tc =
    // Want it?
      (conversion_flags & Dither_Mask) != ThresholdDither && (conversion_flags & DitherMode_Mask) != AvoidDither &&
      // Need it?
        bppc < 24 && !d8 &&
...
#define GET_PIXEL_DITHER_TC \
    ...
    for (i = 1; i < (uint) ncols; i++) { // sort pixels
      ...
      if ((i & 1) || i < 10) { // sort on max distance
        for (int j = 0; j < ncols; j++) {
          px = &pixarr[j];
          if (px->use) {
            dist = (px->r - r) * (px->r - r) + (px->g - g) * (px->g - g) + (px->b - b) * (px->b - b);
            if (px->mindist > dist)
              px->mindist = dist;
  ...
  if (!hd) { // create new pixmap
    hd = (HANDLE) XCreatePixmap(x11Display(), RootWindow(x11Display(), x11Screen()), w, h, dd);
  ...
  if (image.hasAlphaBuffer()) {
    ...
    if (qt_use_xrender && qt_has_xft && alphamap) {
      ...
      data->alphapm->hd = XCreatePixmap(x11Display(), RootWindow(x11Display(), x11Screen()), w, h, 8);

      data->alphapm->rendhd = (HANDLE) XftDrawCreateAlpha(x11Display(), data->alphapm->hd, 8);

      XImage *axi = XCreateImage(x11Display(), (Visual *) x11Visual(), 8, ZPixmap, 0, 0, w, h, 8, 0);
...
QPixmap QPixmap::grabWindow(WId window, int x, int y, int w, int h)
{
  ...
  if (window_attr.depth == root_attr.depth) {
    ...
    WId unused;
    if (!XTranslateCoordinates(dpy, window, window_attr.root, x, y, &x, &y, &unused))
      return QPixmap();
...
QPixmap QPixmap::xForm(const QWMatrix &matrix) const
{
  ...
  if (h == 0 || w == 0 || !invertible || QABS(scaledWidth) >= 32768 || QABS(scaledHeight) >= 32768) { // error, return null pixmap
    QPixmap pm;
  ...
  XImage *xi = (XImage*) data->ximage; // any cached ximage?
  if (!xi)
    xi = XGetImage(x11Display(), handle(), 0, 0, ws, hs, AllPlanes, depth1 ? XYPixmap : ZPixmap);
  ...
  if (depth1) { // mono bitmap
...
#endif
    xi = XCreateImage(dpy, (Visual *) x11Visual(), x11Depth(), ZPixmap, 0, (char *) dptr, w, h, 32, 0);
    ...
    if (qt_use_xrender && qt_has_xft && data->alphapm) { // xform the alpha channel
      XImage *axi = 0;
      if ((axi = XGetImage(x11Display(), data->alphapm->handle(), 0, 0, ws, hs, AllPlanes, ZPixmap))) {
        ...
        if (qt_xForm_helper(mat, axi->xoffset, type, bpp, dptr, w, 0, h, sptr, sbpl, ws, hs)) {
          ...
          // create 8bpp pixmap and render picture
          pm.data->alphapm->hd = XCreatePixmap(x11Display(), RootWindow(x11Display(), x11Screen()), w, h, 8);

          pm.data->alphapm->rendhd = (HANDLE) XftDrawCreateAlpha(x11Display(), pm.data->alphapm->hd, 8);

          XImage *axi2 = XCreateImage(x11Display(), (Visual *) x11Visual(), 8, ZPixmap, 0, (char *) dptr, w, h, 8, 0);
...
Q_EXPORT void copyBlt(QPixmap *dst, int dx, int dy, const QPixmap *src, int sx, int sy, int sw, int sh)
{
  ...
  if (src->data->mask) {
    ...
    bitBlt(dst->data->mask, dx, dy, src->data->mask, sx, sy, sw, sh, Qt::CopyROP, TRUE );
  }
  ...
  if (!dst->data->alphapm) {
    ...
    // create 8bpp pixmap and render picture
    dst->data->alphapm->hd = XCreatePixmap(dst->x11Display(), RootWindow(dst->x11Display(), dst->x11Screen()),
                                           dst->width(), dst->height(), 8);

    // new alpha pixmaps should be fully opaque by default
    do_init = TRUE;

    dst->data->alphapm->rendhd = (Qt::HANDLE) XftDrawCreateAlpha(dst->x11Display(), dst->data->alphapm->hd, 8);
  }
  ...
  if (do_init) {
    ...
    XFillRectangle(dst->x11Display(), dst->data->alphapm->hd, gc, 0, 0, dst->data->alphapm->data->w,
                   dst->data->alphapm->data->h);
  }

  XCopyArea(dst->x11Display(), src->data->alphapm->hd, dst->data->alphapm->hd, gc, sx, sy, sw, sh, dx, dy);
...
// ### AbanQ
/*
 * Paul Hsieh's hash  :  http://www.azillionmonkeys.com/qed/hash.html
 ****************************************************************************/
#define get16bits(d) (*((const uint16_t *) (d)))

static inline uint32_t SuperFastHash(register const char *data, register int len)
{
...
uint32_t QPixmap::hashKey() const
{
  if (isNull())
    return 0;
  XImage *xi = (XImage *) data->ximage;
  if (!xi)
    xi = XGetImage(x11Display(), hd, 0, 0, width(), height(), AllPlanes, ((depth() == 1) ? XYPixmap : ZPixmap));
  else if (data->hashKey_)
    return data->hashKey_;
  if (!xi)
    return 0;

  uint32_t hash = SuperFastHash(xi->data, xi->bytes_per_line * xi->height);
...
uint32_t widgetImageHashKey(QWidget *w, int *state = 0)
{
  Display *dpy = w->x11Display();
  WId wid = w->winId();
  XWindowAttributes a;

  XGetWindowAttributes(dpy, wid, &a);
  if (state)
    *state = a.map_state;
  if (a.map_state != IsViewable)
    return 0;

  XImage *xi = XGetImage(dpy, wid, 0, 0, a.width, a.height, AllPlanes, ZPixmap);
  if (!xi)
    return 0;
