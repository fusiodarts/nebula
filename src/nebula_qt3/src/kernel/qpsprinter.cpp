/**********************************************************************
** $Id: qt/qpsprinter.cpp   3.3.8   edited Jan 29 17:22 $
**
...
#define AQ_PS_FILE        QString(getenv("HOME")) + "/.flxcache/print.ps"
#define AQ_PDF_FILE       QString(getenv("HOME")) + "/.flxcache/abanq_web.pdf"
#define AQ_OPTIM_PDF_FILE QString(getenv("HOME")) + "/.flxcache/abanq_web_print.pdf"
static inline bool aq_ps_to_pdf(QProcess *proc, bool noOpt = true)
{
  QString inPS(AQ_PS_FILE);
  QString outPDF(AQ_PDF_FILE);
  if (noOpt) {
    QFile::remove(outPDF);
    proc->addArgument("gs");
    proc->addArgument("-q");
    ...
    proc->addArgument(outPDF);
    proc->addArgument(AQ_OPTIM_PDF_FILE);
    if (!proc->start())
      return false;
  }
  return true;
...
bool QPSPrinter::cmd(int c , QPainter *paint, QPDevCmdParam *p)
{
  ...
  if (c == PdcEnd) {                          // painting done
    ...
    if (aq_ps_to_pdf(proc))
      ::waitpid(proc->processIdentifier(), 0, 0);
    delete proc;
    proc = new QProcess();
    if (aq_ps_to_pdf(proc, false))
      ::waitpid(proc->processIdentifier(), 0, 0);
    delete proc;
    aq_notify_gui_args_event(QApplication::desktop(), "xdwl", AQ_OPTIM_PDF_FILE);
  }
