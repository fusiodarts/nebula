...
QSharedDoubleBufferCleaner::QSharedDoubleBufferCleaner(void) :
  QObject(0, "internal shared double buffer cleanup object"), timer_id(-1)
{
}
...
QSharedDoubleBuffer::QSharedDoubleBuffer(DBFlags f) :
  wid(0), rx(0), ry(0), rw(0), rh(0), flags(f), state(0), p(0), external_p(0), pix(0)
{
}
 ...
 */
QSharedDoubleBuffer::QSharedDoubleBuffer(QWidget* widget, int x, int y, int w, int h, DBFlags f) :
  wid(0), rx(0), ry(0), rw(0), rh(0), flags(f), state(0), p(0), external_p(0), pix(0)
{
...
/* \internal
 ...
 \sa DoubleBufferFlags begin()
 */
QSharedDoubleBuffer::QSharedDoubleBuffer(QPainter* painter, int x, int y, int w, int h, DBFlags f) :
  wid(0), rx(0), ry(0), rw(0), rh(0), flags(f), state(0), p(0), external_p(0), pix(0)
{
...
QPixmap *QSharedDoubleBuffer::getPixmap()
{
  ...
  if (rw <= 0 || rh <= 0 || (hardLimitWidth > 0 && rw >= hardLimitWidth) || (hardLimitHeight > 0 && rh
      >= hardLimitHeight)) {
  ...
  if (rw >= sharedLimitWidth || rh >= sharedLimitHeight) {
    if ( flags & Force ) {
      ...
      } else if (qdb_force_pixmap->width() < rw || qdb_force_pixmap->height() < rh) {
  ...
  } else if (qdb_shared_pixmap->width() < rw || qdb_shared_pixmap->height() < rh) {
