...
extern void aq_notify_gui_args_event(QWidget *, const QString &, const QStringList &);
extern void aq_draw_pixmap(QWidget*, int, int, const QPixmap *, int, int, int, int);

static inline QString aq_pen_tostr(const QPen &pen)
{
  QString ret;
  if (pen.style() == 1 && pen.width() == 0 && pen.joinStyle() == 0 && pen.capStyle() == 0) {
    ret += "";
  } else {
    ret += QString::number(pen.style());
    ret += ";" + QString::number(pen.width());
    ret += ";" + QString::number(pen.joinStyle());
    ret += ";" + QString::number(pen.capStyle());
  }
  if (!ret.isEmpty())
    ret += ";";
  ret += pen.color().name();
...
static inline QString aq_brush_tostr(const QBrush &bru)
{
  QString ret;
  if (bru.style() == 1)
    ret += "";
  else
    ret += QString::number(bru.style());
  if (!ret.isEmpty())
    ret += ";";
  ret += bru.color().name();
  return ret;
...
static inline void aq_draw_lines(XPoint *pts, int npts, QWidget *wi, const QPen &pen, const QBrush &bru)
{
  if (npts < 2)
    return;

  XPoint *points = pts;
  int lastx = points->x, lasty = points->y;
  QString ee("m;" + AQ_STR_NUMBER_COAL(lastx) + ";" + AQ_STR_NUMBER_COAL(lasty));
  ++points;

  for (int i = 1; i < npts; ++i) {
    if (lastx != points->x || lasty != points->y)
      ee += ";" + AQ_STR_NUMBER_COAL(points->x) + ";" + AQ_STR_NUMBER_COAL(points->y);
    lastx = points->x;
    lasty = points->y;
    ++points;
  }

  aq_notify_gui_args_event(wi, "xpat", QStringList() << ee << (pen.style() != Qt::NoPen ? aq_pen_tostr(pen)
                                                                                        : QString::null)
    << (bru.style() != Qt::NoBrush ? aq_brush_tostr(bru) : QString::null));
}

static inline void aq_draw_segments(XSegment *segs, int nsegs, QWidget *wi, const QPen &pen)
{
  XSegment *sgs = segs;
  QString ee;
  ...
  for (int i = 0; i < nsegs; ++i) {
    if (i)
      ee += ";";
    ee += "m;" + AQ_STR_NUMBER_COAL(sgs->x1) + ";" + AQ_STR_NUMBER_COAL(sgs->y1);
    ee += ";" + AQ_STR_NUMBER_COAL(sgs->x2) + ";" + AQ_STR_NUMBER_COAL(sgs->y2);
    ++sgs;
  }

  aq_notify_gui_args_event(wi, "xpat", QStringList() << ee << aq_pen_tostr(pen));
}

static inline QString aq_get_path(const QRegion &r)
{
  int num;
  XRectangle *rr = (XRectangle *) qt_getClipRects(r, num);
  ...
  for (int i = 0; i < num; ++i) {
    if (i)
      ee += ";";
    ee += "m;" + AQ_STR_NUMBER_COAL(rr->x) + ";" + AQ_STR_NUMBER_COAL(rr->y) + ";";
    ee += AQ_STR_NUMBER_COAL(rr->x + rr->width) + ";" + AQ_STR_NUMBER_COAL(rr->y) + ";";
    ee += AQ_STR_NUMBER_COAL(rr->x + rr->width) + ";" + AQ_STR_NUMBER_COAL(rr->y + rr->height) + ";";
    ee += AQ_STR_NUMBER_COAL(rr->x) + ";" + AQ_STR_NUMBER_COAL(rr->y + rr->height) + ";";
    ee += AQ_STR_NUMBER_COAL(rr->x) + ";" + AQ_STR_NUMBER_COAL(rr->y);
    ++rr;
  }
...
void aq_bitblt(QWidget *wi, int dx, int dy, const QPaintDevice *src, int sx, int sy, int sw, int sh)
{
  int ts = src->devType();
  if (ts == QInternal::Widget) {
    const QWidget *swi = (const QWidget *) src;

#ifdef AQ_WID_PM
    if (!wi->paintingActive())
    wi->aq_pm()->detach();
    bitBlt(wi->aq_pm_, dx, dy, swi, sx, sy, sw, sh);
#endif

    if (wi == swi) {
      aq_notify_gui_args_event(wi, "xcnv", QStringList() << AQ_STR_NUMBER_COAL(dx) << AQ_STR_NUMBER_COAL(dy)
        << AQ_STR_NUMBER_COAL(sx) << AQ_STR_NUMBER_COAL(sy) << AQ_STR_NUMBER_COAL(sw) << AQ_STR_NUMBER_COAL(sh));
    } else {
      aq_notify_gui_args_event(wi, "xcnv", QStringList() << AQ_STR_NUMBER_COAL(dx) << AQ_STR_NUMBER_COAL(dy)
        << AQ_STR_NUMBER_COAL(sx) << AQ_STR_NUMBER_COAL(sy) << AQ_STR_NUMBER_COAL(sw) << AQ_STR_NUMBER_COAL(sh)
        << QString::number(reinterpret_cast<Q_ULLONG> (swi), 36));
    }
  } else if (ts == QInternal::Pixmap) {
    const QPixmap *pix = (const QPixmap *) src;

#ifdef AQ_WID_PM
    if (!wi->paintingActive())
    wi->aq_pm()->detach();
    copyBlt(wi->aq_pm_, dx, dy, pix, sx, sy, sw, sh);
#endif

    if (pix->width() != sw || pix->height() != sh) {
      QPixmap aqpix(sw, sh);
      copyBlt(&aqpix, 0, 0, pix, sx, sy, sw, sh);
      aq_draw_pixmap(wi, dx, dy, &aqpix, 0, 0, sw, sh);
    } else
      aq_draw_pixmap(wi, dx, dy, pix, sx, sy, sw, sh);
  } else {
    qWarning("aq_bitblt src device ¿? %d", ts);
  }
}

void aq_erase_region(QWidget* w, const QRegion& region)
{
  QRegion reg = region;
...
#ifdef AQ_WID_PM
    pterpm.fillRect(rr, w->eraseColor());
#endif
    if (rr.width() > 1 && rr.height() > 1)
      aq_notify_gui_args_event(w, "xcla", QStringList() << AQ_STR_NUMBER_COAL(rr.x()) << AQ_STR_NUMBER_COAL(rr.y())
        << AQ_STR_NUMBER_COAL(rr.width()) << AQ_STR_NUMBER_COAL(rr.height()));
  }
}
// ### AbanQ
...
void qt_erase_region(QWidget* w, const QRegion& region)
{
  ...
  // ### AbanQ
  const QPixmap *bgpm = w->backgroundPixmap();

  if (QPainter::redirect(w) || (bgpm && !bgpm->isNull()) || (!w->isTopLevel() && bgpm && w->backgroundOrigin()
    ...
    if (bgpm)
      p.drawTiledPixmap(0, 0, w->width(), w->height(), *bgpm, ox, oy);
    else
  ...
  for (uint i = 0; i < r.size(); i++) {
    ...
    XClearArea(w->x11Display(), w->winId(), rr.x(), rr.y(), rr.width(), rr.height(), False);
  }
}

void qt_erase_rect(QWidget* w, const QRect& r)
{
  if (QPainter::redirect(w) || w == paintEventDevice || w->backgroundOrigin() != QWidget::WidgetOrigin) {
    qt_erase_region(w, r);
  } else {
    ...
    {
      QPainter pterpm(w->aq_pm());
      pterpm.fillRect(r, w->eraseColor());
    }
#endif
    if (r.width() > 1 && r.height() > 1)
      aq_notify_gui_args_event(w, "xcla", QStringList() << AQ_STR_NUMBER_COAL(r.x()) << AQ_STR_NUMBER_COAL(r.y())
        << AQ_STR_NUMBER_COAL(r.width()) << AQ_STR_NUMBER_COAL(r.height()));
...
static GC alloc_gc(Display *dpy, int scrn, Drawable hd, bool monochrome = FALSE, bool privateGC = FALSE)
{
...
static bool obtain_gc(void **ref, GC *gc, uint pix, Display *dpy, int scrn, Qt::HANDLE hd, uint painter_clip_serial)
{
  if (!gc_cache_init)
    init_gc_cache();
...
void QPainter::updatePen()
{
  ...
  int ps = cpen.style();
  bool cacheIt = !testf(ClipOn | MonoDev | NoCache) && (ps == NoPen || ps == SolidLine) && cpen.width() == 0 && rop
    == CopyROP;
  ...
  if (cacheIt) {
    ...
    obtained = obtain_gc(&penRef, &gc, cpen.color().pixel(scrn), dpy, scrn, hd, clip_serial);
    if (!obtained && !penRef)
  ...
  XSetLineAttributes(dpy, gc, (!allow_zero_lw && cpen.width() == 0) ? 1 : cpen.width(), s, cp, jn);
}
...
void QPainter::updateBrush()
{
  ...
  static const uchar ver_pat[] = { // vertical pattern
    0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08,
      0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82,
      0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20,
      0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20 };
  static const uchar cross_pat[] = { // cross pattern
    0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0xff, 0xff, 0xff, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08,
      0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0xff, 0xff, 0xff, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82,
      0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0xff, 0xff, 0xff, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20,
      0x08, 0x82, 0x20, 0x08, 0x82, 0x20, 0xff, 0xff, 0xff, 0x08, 0x82, 0x20, 0x08, 0x82, 0x20 };
  static const uchar bdiag_pat[] = { // backward diagonal pattern
    0x20, 0x20, 0x10, 0x10, 0x08, 0x08, 0x04, 0x04, 0x02, 0x02, 0x01, 0x01, 0x80, 0x80, 0x40, 0x40, 0x20, 0x20, 0x10,
      0x10, 0x08, 0x08, 0x04, 0x04, 0x02, 0x02, 0x01, 0x01, 0x80, 0x80, 0x40, 0x40 };
  static const uchar fdiag_pat[] = { // forward diagonal pattern
    0x02, 0x02, 0x04, 0x04, 0x08, 0x08, 0x10, 0x10, 0x20, 0x20, 0x40, 0x40, 0x80, 0x80, 0x01, 0x01, 0x02, 0x02, 0x04,
      0x04, 0x08, 0x08, 0x10, 0x10, 0x20, 0x20, 0x40, 0x40, 0x80, 0x80, 0x01, 0x01 };
  static const uchar dcross_pat[] = { // diagonal cross pattern
    0x22, 0x22, 0x14, 0x14, 0x08, 0x08, 0x14, 0x14, 0x22, 0x22, 0x41, 0x41, 0x80, 0x80, 0x41, 0x41, 0x22, 0x22, 0x14,
      0x14, 0x08, 0x08, 0x14, 0x14, 0x22, 0x22, 0x41, 0x41, 0x80, 0x80, 0x41, 0x41 };
  static const uchar * const pat_tbl[] = { dense1_pat, dense2_pat, dense3_pat, dense4_pat, dense5_pat, dense6_pat,
                                           dense7_pat, hor_pat, ver_pat, cross_pat, bdiag_pat, fdiag_pat, dcross_pat };
  ...
  int bs = cbrush.style();
  bool cacheIt = !testf(ClipOn | MonoDev | NoCache) && (bs == NoBrush || bs == SolidPattern) && bro.x() == 0 && bro.y()
    == 0 && rop == CopyROP;
  ...
  if (cacheIt) {
    ...
    obtained = obtain_gc(&brushRef, &gc_brush, cbrush.color().pixel(scrn), dpy, scrn, hd, clip_serial);
...
bool QPainter::begin(const QPaintDevice *pd, bool unclipped)
{
  ...
  if (dt == QInternal::Widget) { // device is a widget
    ...
    // ### AbanQ
    QString ee;
    bool isUnclipped = (unclipped || w->testWFlags(WPaintUnclipped));
    if (!isUnclipped && pdev == paintEventDevice && paintEventClipRegion)
      ee = aq_get_path(*paintEventClipRegion);
#ifdef AQ_WID_PM
    QPixmap *wpm = w->aq_pm();
    if (wpm->painters == 0)
    w->aq_ppm_.begin(wpm, unclipped);
    else
    wpm->painters++;
    if (!ee.isEmpty()) {
      w->aq_ppm_.crgn = *paintEventClipRegion;
      w->aq_ppm_.setClipping(true);
    }
#endif
    aq_notify_gui_args_event(w, "xbeg", ee);
...
bool QPainter::end() // end painting
{
  ...
  if (w) {
#ifdef AQ_WID_PM
    if (w->aq_pm()->painters == 1)
    w->aq_ppm_.end();
    else
    w->aq_pm()->painters--;
#endif
    aq_notify_gui_args_event(w, "xend", QStringList());
  }
...
void QPainter::setClipping(bool enable)
{
  ...
  if (enable) {
    ...
    if (w) {
#ifdef AQ_WID_PM
      w->aq_ppm_.setf(ClipOn, false);
      w->aq_ppm_.crgn = rgn;
      w->aq_ppm_.setClipping(true);
#endif
      QString ee(aq_get_path(rgn));
      if (!ee.isEmpty())
        aq_notify_gui_args_event(w, "xscl", ee);
    }
    ...
    if (w) {
      ...
      w->aq_ppm_.setClipping(false);
#endif
      aq_notify_gui_args_event(w, "xccl", QStringList());
    }
...
void QPainter::drawPolyInternal(const QPointArray &a, bool close)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    w = (QWidget*) pdev;
#ifdef AQ_WID_PM
    w->aq_ppm_.setBrush(cbrush);
    w->aq_ppm_.setPen(cpen);
    w->aq_ppm_.drawPolyInternal(a, close);
#endif
  }
  ...
  if (close && cbrush.style() != NoBrush) { // draw filled polygon
    ...
    // ### AbanQ
    if (w)
      aq_draw_lines((XPoint*) a.shortPoints(), a.size(), w, QPen(), cbrush);

    XFillPolygon(dpy, hd, gc_brush, (XPoint*) a.shortPoints(), a.size(), Nonconvex, CoordModeOrigin);
    if (cpen.style() == NoPen) { // draw fake outline

      // ### AbanQ
      if (w)
        aq_draw_lines((XPoint*) a.shortPoints(), a.size(), w, QPen(cbrush.color()), QBrush());

      XDrawLines(dpy, hd, gc_brush, (XPoint*) a.shortPoints(), a.size(), CoordModeOrigin);
      if (do_close) {

        // ### AbanQ
        if (w) {
          aq_notify_gui_args_event(w, "xlin", QStringList() << AQ_STR_NUMBER_COAL(x1) << AQ_STR_NUMBER_COAL(y1)
            << AQ_STR_NUMBER_COAL(x2) << AQ_STR_NUMBER_COAL(y2) << aq_pen_tostr(QPen(cbrush.color())));
        }
  ...
  if (cpen.style() != NoPen) { // draw outline
    // ### AbanQ
    if (w)
      aq_draw_lines((XPoint*) a.shortPoints(), a.size(), w, cpen, QBrush());

    XDrawLines(dpy, hd, gc, (XPoint*) a.shortPoints(), a.size(), CoordModeOrigin);
    if (do_close) {

      // ### AbanQ
      if (w) {
        aq_notify_gui_args_event(w, "xlin", QStringList() << AQ_STR_NUMBER_COAL(x1) << AQ_STR_NUMBER_COAL(y1)
          << AQ_STR_NUMBER_COAL(x2) << AQ_STR_NUMBER_COAL(y2) << aq_pen_tostr(cpen));
      }
...
#ifdef AQ_WID_PM
      w->aq_ppm_.setPen(cpen);
      w->aq_ppm_.drawPoint(x, y);
#endif
      aq_notify_gui_args_event(w, "xpnt", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
        << aq_pen_tostr(cpen));
    }
...
void QPainter::drawPoints(const QPointArray& a, int index, int npoints)
{
  ...
  if (cpen.style() != NoPen) {
    ...
    if (pdev->devType() == QInternal::Widget) {
      ...
      for (int i = 0; i < npoints; i++) {
#ifdef AQ_WID_PM
        w->aq_ppm_.drawPoint(pa[index + i].x(), pa[index + i].y());
#endif
        aq_notify_gui_args_event(w, "xpnt", QStringList() << AQ_STR_NUMBER_COAL(pa[index + i].x())
          << AQ_STR_NUMBER_COAL(pa[index + i].y()) << aq_pen_tostr(cpen));
      }
    }

    XDrawPoints(dpy, hd, gc, (XPoint*) (pa.shortPoints(index, npoints)), npoints, CoordModeOrigin);
  }
}
...
#ifdef AQ_WID_PM
      w->aq_ppm_.setPen(cpen);
      w->aq_ppm_.drawLine(cx, cy, x, y);
#endif
      aq_notify_gui_args_event(w, "xlin", QStringList() << AQ_STR_NUMBER_COAL(cx) << AQ_STR_NUMBER_COAL(cy)
        << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y) << aq_pen_tostr(cpen));
    }
...
#ifdef AQ_WID_PM
      w->aq_ppm_.setPen(cpen);
      w->aq_ppm_.drawLine(x1, y1, x2, y2);
#endif
      aq_notify_gui_args_event(w, "xlin", QStringList() << AQ_STR_NUMBER_COAL(x1) << AQ_STR_NUMBER_COAL(y1)
        << AQ_STR_NUMBER_COAL(x2) << AQ_STR_NUMBER_COAL(y2) << aq_pen_tostr(cpen));
    }
...
void QPainter::drawRect(int x, int y, int w, int h)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    wi = (QWidget*) pdev;
#ifdef AQ_WID_PM
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawRect(x, y, w, h);
#endif
  }
  ...
  if (cbrush.style() != NoBrush) {
    if (cpen.style() == NoPen) {
      ...
      // ### AbanQ
      if (wi) {
        aq_notify_gui_args_event(wi, "xrec", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
          << AQ_STR_NUMBER_COAL(w) << AQ_STR_NUMBER_COAL(h) << QString::null << aq_brush_tostr(cbrush));
      }
    ...
    if (w > lw && h > lw) {
      ...
      // ### AbanQ
      if (wi) {
        aq_notify_gui_args_event(wi, "xrec", QStringList() << AQ_STR_NUMBER_COAL(x + lw2)
          << AQ_STR_NUMBER_COAL(y + lw2) << AQ_STR_NUMBER_COAL(w - lw - 1) << AQ_STR_NUMBER_COAL(h - lw - 1)
          << QString::null << aq_brush_tostr(cbrush));
      }
  ...
  if (cpen.style() != NoPen) {
    ...
    if (wi) {
      aq_notify_gui_args_event(wi, "xrec", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
        << AQ_STR_NUMBER_COAL(w - 1) << AQ_STR_NUMBER_COAL(h - 1) << aq_pen_tostr(cpen));
    }
...
void QPainter::drawWinFocusRect(int x, int y, int w, int h, bool xorPaint, const QColor &bgColor)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawWinFocusRect(x, y, w, h, xorPaint, bgColor);
#endif
  }
...
void QPainter::drawRoundRect(int x, int y, int w, int h, int xRnd, int yRnd)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawRoundRect(x, y, w, h, xRnd, yRnd);
#endif
  }
...
#define SET_SEG(xp1, yp1, xp2, yp2) \
    ...
    // ### AbanQ
    if (wi)
      aq_draw_segments((XSegment*) segs, 4, wi, cpen);
...
void QPainter::drawEllipse(int x, int y, int w, int h)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawEllipse(x, y, w, h);
#endif
  }
  ...
  if (w == 1 && h == 1) {
    ...
    if (wi) {
      ...
      if (cpen.style() == Qt::NoPen) {
        ...
        pe.setColor(cbrush.color());
      }

      aq_notify_gui_args_event(wi, "xpnt", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
        << aq_pen_tostr(pe));
    }
  ...
  // ### AbanQ
  if (wi) {
    aq_notify_gui_args_event(wi, "xarc", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
      << AQ_STR_NUMBER_COAL(w) << AQ_STR_NUMBER_COAL(h) << "0" << AQ_STR_NUMBER_COAL(360 * 16) << (cpen.style()
      != Qt::NoPen ? aq_pen_tostr(cpen) : QString::null) << (cbrush.style() != Qt::NoBrush ? aq_brush_tostr(cbrush)
                                                                                           : QString::null));
  }
...
void QPainter::drawArc(int x, int y, int w, int h, int a, int alen)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawArc(x, y, w, h, a, alen);
#endif
  }
  ...
  if (cpen.style() != NoPen) {
    ...
    if (wi) {
      aq_notify_gui_args_event(wi, "xarc", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
        << AQ_STR_NUMBER_COAL(w) << AQ_STR_NUMBER_COAL(h) << AQ_STR_NUMBER_COAL(a) << AQ_STR_NUMBER_COAL(alen)
        << aq_pen_tostr(cpen));
    }
...
void QPainter::drawPie(int x, int y, int w, int h, int a, int alen)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawPie(x, y, w, h, a, alen);
#endif
  }
  ...
  if (!nopen) { // draw pie outline
    ...
    // ### AbanQ
    if (wi) {
      aq_notify_gui_args_event(wi, "xlin", QStringList() << AQ_STR_NUMBER_COAL(xic) << AQ_STR_NUMBER_COAL(yic)
        << AQ_STR_NUMBER_COAL(qRound(xc + qcos(ra1) * w2)) << AQ_STR_NUMBER_COAL(qRound(yc - qsin(ra1) * h2))
        << aq_pen_tostr(cpen));

      aq_notify_gui_args_event(wi, "xlin", QStringList() << AQ_STR_NUMBER_COAL(xic) << AQ_STR_NUMBER_COAL(yic)
        << AQ_STR_NUMBER_COAL(qRound(xc + qcos(ra2) * w2)) << AQ_STR_NUMBER_COAL(qRound(yc - qsin(ra2) * h2))
        << aq_pen_tostr(cpen));
    ...
    XDrawLine(dpy, hd, g, xic, yic, qRound(xc + qcos(ra1) * w2), qRound(yc - qsin(ra1) * h2));
    XDrawLine(dpy, hd, g, xic, yic, qRound(xc + qcos(ra2) * w2), qRound(yc - qsin(ra2) * h2));
...
void QPainter::drawChord(int x, int y, int w, int h, int a, int alen)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawChord(x, y, w, h, a, alen);
#endif
  }
  ...
  if (!nopen) { // draw chord outline
    ...
    // ### AbanQ
    if (wi) {
      aq_notify_gui_args_event(wi, "xlin", QStringList() << AQ_STR_NUMBER_COAL(qRound(xc + qcos(ra1) * w2))
        << AQ_STR_NUMBER_COAL(qRound(yc - qsin(ra1) * h2)) << AQ_STR_NUMBER_COAL(qRound(xc + qcos(ra2) * w2))
        << AQ_STR_NUMBER_COAL(qRound(yc - qsin(ra2) * h2)) << aq_pen_tostr(cpen));
    ...
    XDrawLine(dpy, hd, g, qRound(xc + qcos(ra1) * w2), qRound(yc - qsin(ra1) * h2), qRound(xc + qcos(ra2) * w2),
              qRound(yc - qsin(ra2) * h2));
    XDrawArc(dpy, hd, g, x, y, w, h, a * 4, alen * 4);
  }
  XSetArcMode(dpy, gc_brush, ArcPieSlice);
}
...
void QPainter::drawLineSegments(const QPointArray &a, int index, int nlines)
{
  ...
  if (cpen.style() != NoPen) {
    ...
    // ### AbanQ
    if (pdev->devType() == QInternal::Widget) {
      QWidget *wi = (QWidget*) pdev;
#ifdef AQ_WID_PM
      wi->aq_ppm_.setPen(cpen);
      wi->aq_ppm_.drawLineSegments(a, index, nlines);
#endif
      aq_draw_segments((XSegment*) (pa.shortPoints(index, nlines * 2)), nlines, wi, cpen);
    }

    XDrawSegments(dpy, hd, gc, (XSegment*) (pa.shortPoints(index, nlines * 2)), nlines);
  }
}
...
void QPainter::drawPolyline(const QPointArray &a, int index, int npoints)
{
  ...
  if (cpen.style() != NoPen) {
...
#ifdef AQ_WID_PM
      wi->aq_ppm_.setPen(cpen);
      wi->aq_ppm_.drawPolyline(a, index, npoints);
#endif
    }
    ...
    while (npoints > 65535) {
      ...
      // ### AbanQ
      if (wi)
        aq_draw_lines((XPoint*) (pa.shortPoints(index, 65535)), 65535, wi, cpen, QBrush());

      XDrawLines(dpy, hd, gc, (XPoint*) (pa.shortPoints(index, 65535)), 65535, CoordModeOrigin);
      npoints -= 65535;
      index += 65535;
    }
    ...
    // ### AbanQ
    if (wi)
      aq_draw_lines((XPoint*) (pa.shortPoints(index, npoints)), npoints, wi, cpen, QBrush());

    XDrawLines(dpy, hd, gc, (XPoint*) (pa.shortPoints(index, npoints)), npoints, CoordModeOrigin);
  }
}
...
void QPainter::drawPolygon(const QPointArray &a, bool winding, int index, int npoints)
{
  ...
  if (pdev->devType() == QInternal::Widget) {
    ...
    wi->aq_ppm_.setBrush(cbrush);
    wi->aq_ppm_.setPen(cpen);
    wi->aq_ppm_.drawPolygon(a, winding, index, npoints);
#endif
    aq_draw_lines((XPoint*) (pa.shortPoints(index, npoints)), npoints, wi, cpen, cbrush);
  }

  if (cbrush.style() != NoBrush) { // draw filled polygon
    XFillPolygon(dpy, hd, gc_brush, (XPoint*) (pa.shortPoints(index, npoints)), npoints, global_polygon_shape,
                 CoordModeOrigin);
  }
  if (cpen.style() != NoPen) { // draw outline
    XDrawLines(dpy, hd, gc, (XPoint*) (pa.shortPoints(index, npoints)), npoints, CoordModeOrigin);
...
/*!
 ...
 Qt 4.
 */
void QPainter::drawConvexPolygon(const QPointArray &pa, int index, int npoints)
{
...
#ifdef AQ_WID_PM
      wi->aq_ppm_.setPen(cpen);
      wi->aq_ppm_.drawCubicBezier(a, index);
#endif
      aq_draw_lines((XPoint*) pa.shortPoints(), pa.size(), wi, cpen, QBrush());
    }

    XDrawLines(dpy, hd, gc, (XPoint*) pa.shortPoints(), pa.size(), CoordModeOrigin);
  }
}
...
void QPainter::drawPixmap(int x, int y, const QPixmap &pixmap, int sx, int sy, int sw, int sh)
{
  ...
  if (testf(ExtDev | VxF | WxF)) {
    if (testf(ExtDev) || txop == TxScale || txop == TxRotShear) {
      if (sx != 0 || sy != 0 || sw != pixmap.width() || sh != pixmap.height()) {
        QPixmap tmp(sw, sh, pixmap.depth());
        bitBlt(&tmp, 0, 0, &pixmap, sx, sy, sw, sh, CopyROP, TRUE );
        if (pixmap.mask()) {
          QBitmap mask(sw, sh);
          bitBlt(&mask, 0, 0, pixmap.mask(), sx, sy, sw, sh, CopyROP, TRUE );
          tmp.setMask(mask);
        }
        drawPixmap(x, y, tmp);
        return;
      ...
      if (txop == TxScale || txop == TxRotShear) {
        QWMatrix mat(m11(), m12(), m21(), m22(), dx(), dy());
  ...
  if (mono) {
    ...
    if (pict && pixmap.x11RenderHandle() && alpha && alpha->x11RenderHandle()) {
      XRenderComposite(dpy, PictOpOver, pixmap.x11RenderHandle(), alpha->x11RenderHandle(), pict, sx, sy, sx, sy, x, y,
                       sw, sh);
    } else
  ...
  // ### AbanQ
  if (pdev->devType() == QInternal::Widget)
    aq_bitblt((QWidget*) pdev, x, y, &pixmap, sx, sy, sw, sh);
}

/* Internal, used by drawTiledPixmap */

static void drawTile(QPainter *p, int x, int y, int w, int h, const QPixmap &pixmap, int xOffset, int yOffset)
{
...
void QPainter::drawTiledPixmap(int x, int y, int w, int h, const QPixmap &pixmap, int sx, int sy)
{
   ...
   */
  QBitmap *mask = (QBitmap *) pixmap.mask();
  if (!testf(ExtDev) && txop <= TxTranslate && pixmap.depth() > 1 && mask == 0) {
    if (txop == TxTranslate)
    map(x, y, &x, &y);
    ...
    if (pict && pixmap.x11RenderHandle() && alpha && alpha->x11RenderHandle()) {
      ...
      while (yPos < y + h) {
        ...
        while (xPos < x + w) {
          ...
          if (xPos + drawW > x + w) // Cropping last column
          drawW = x + w - xPos;
          XRenderComposite(dpy, PictOpOver, pixmap.x11RenderHandle(), alpha->x11RenderHandle(), pict, xOff, yOff, xOff,
            yOff, xPos, yPos, drawW, drawH);
          xPos += drawW;
          xOff = 0;
        }
        yPos += drawH;
...
void qt_draw_transformed_rect(QPainter *p, int x, int y, int w, int h, bool fill)
{
  ...
  if (fill) {
    ...
    if (wi) {
#ifdef AQ_WID_PM
      XFillPolygon(wi->aq_ppm_.dpy, wi->aq_ppm_.hd, p->gc, points, 4, Convex, CoordModeOrigin);
#endif
      aq_draw_lines(points, 4, wi, p->bg_col, p->bg_col);
    }
    ...
    if (wi) {
#ifdef AQ_WID_PM
      XDrawLines(wi->aq_ppm_.dpy, wi->aq_ppm_.hd, p->gc, points, 5, CoordModeOrigin);
#endif
      aq_draw_lines(points, 5, wi, p->bg_col, QBrush());
    }
...
void QPainter::drawText(int x, int y, const QString &str, int pos, int len, QPainter::TextDirection dir)
{
  ...
  // ### AbanQ
  QRect tbrect(x, y - ascent, right - left, ascent + descent + 1);
  if (bg_mode == OpaqueMode)
    qt_draw_background(this, tbrect.x(), tbrect.y(), tbrect.width(), tbrect.height());
  //  if (bg_mode == OpaqueMode)
  ...
  for (int i = start; i < end; i++) {
    ...
    drawTextItem(x, y - ascent, ti, textFlags, tbrect);
  }
  layout.d = 0;
}
 ...
 */
void QPainter::drawTextItem(int x, int y, const QTextItem &ti, int textFlags, const QRect &brect)
{
  if (testf(ExtDev)) {
    QPDevCmdParam param[2];
    QPoint p(x, y);
  ...
  if (pdev->devType() == QInternal::Widget) {
...
#ifdef AQ_WID_PM
    QRect r(brect.isValid() ? brect : wi->rect());
    int rx, ry, rw, rh;
    r.rect(&rx, &ry, &rw, &rh);
#endif
    ...
    if (testf(VxF | WxF)) {
      map(x, y, &x, &y);
#ifdef AQ_WID_PM
      map(rx, ry, rw, rh, &rx, &ry, &rw, &rh);
#endif
    }

#ifdef AQ_WID_PM
    if (wi->aq_ppm_.testf(ClipOn)) {
      QRect cr_(wi->aq_ppm_.crgn.boundingRect());
      int difArea = (rw * rh) - (cr_.width() * cr_.height());
      if (!cr_.isEmpty() && difArea > 0) {
        QRect r_(rx, ry, rw, rh);
        if (cr_.intersects(r_)) {
          cr_ = cr_.intersect(r_);
          cr_.rect(&rx, &ry, &rw, &rh);
        }
      }
    }
    XCopyArea(dpy, hd, wi->aq_ppm_.hd, wi->aq_ppm_.gc, rx, ry, rw, rh, rx, ry);
#endif

    QFontInfo fi(this);
    QString cssFont(fi.italic() ? "italic " : "normal ");
    cssFont += QString::number(fi.weight() * 8) + " ";
    cssFont += QString::number(fi.pixelSize()) + "px ";
    cssFont += font().family() + ",";
    cssFont += "Helvetica";

    aq_notify_gui_args_event(wi, "xtxt", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
      << ti.engine->string.mid(si->position, ti.engine->length(ti.item)) << AQ_STR_NUMBER_COAL(si->width) << cssFont
      << QString::null << aq_brush_tostr(QBrush(cpen.color())));
  }
}
