...
QPainter::QPainter(const QPaintDevice *pd, const QWidget *copyAttributes, bool unclipped)
{
  init();
...
void QPainter::setWorldMatrix(const QWMatrix &m, bool combine)
{
  ...
  bool identity = wxmat.m11() == 1.0F && wxmat.m22() == 1.0F && wxmat.m12() == 0.0F && wxmat.m21() == 0.0F
...
void QPainter::map(int x, int y, int w, int h, int *rx, int *ry, int *rw, int *rh) const
{
#ifndef QT_NO_TRANSFORMATIONS
  if (qt_old_transformations) {
    switch (txop) {
...
void QPainter::mapInv(int x, int y, int w, int h, int *rx, int *ry, int *rw, int *rh) const
{
#ifndef QT_NO_TRANSFORMATIONS
#if defined(QT_CHECK_STATE)
 ...
 */
void QPainter::drawImage(int x, int y, const QImage & image, int sx, int sy, int sw, int sh, int conversionFlags)
{
#ifdef Q_WS_QWS
  //### Hackish
# ifndef QT_NO_TRANSFORMATIONS
...
{
  QPixmap tmp;
  if (sx == 0 && sy == 0 && (sw < 0 || sw == src->width()) && (sh < 0 || sh == src->height())) {
...
void QPainter::drawText(const QRect &r, int tf, const QString& str, int len, QRect *brect, QTextParag **internal)
{
  if (!isActive())
    return;
  if (len < 0)
  ...
  if (testf(DirtyFont | ExtDev)) {
    ...
    if (testf(ExtDev) && (tf & DontPrint) == 0) {
      ...
      if (pdev->devType() != QInternal::Printer) {
        ...
        return; // QPrinter wants PdcDrawText2
#else
        if (!pdev->cmd(QPaintDevice::PdcDrawText2Formatted, this, param) || !hd)
          return; // QPrinter wants PdcDrawText2
#endif
      }
    }
  ...
  qt_format_text(font(), r, tf, str, len, brect, tabstops, tabarray, tabarraylen, internal, this);
}
...
void qt_format_text(const QFont& font, const QRect &_r, int tf, const QString& str, int len, QRect *brect,
                    int tabstops, int* tabarray, int tabarraylen, QTextParag **, QPainter* painter)
{
  ...
  bool expandtabs = ((tf & Qt::ExpandTabs) && (((tf & Qt::AlignLeft) && !isRightToLeft) || ((tf & Qt::AlignRight)
      && isRightToLeft)));
  ...
  if (text.isEmpty()) {
    ...
    textLayout.beginLayout((haveLineSep || expandtabs || wordbreak) ? QTextLayout::MultiLine
                                                                    : (tf & Qt::DontPrint) ? QTextLayout::NoBidi
                                                                                           : QTextLayout::SingleLine);
  ...
  if (!(tf & QPainter::DontPrint)) {
    ...
    for (int i = 0; i < textLayout.numItems(); i++) {
      ...
      if (ti.y() + h < height)
        ...
        h += fm.leading();
      QRect tbrect(r.x() + lb + ti.x(), r.y() + yoff + ti.y() - ti.ascent(), ti.width(), h);
#if defined(Q_WS_X11) || defined(Q_WS_QWS)
      if (painter->bg_mode == Qt::OpaqueMode)
        qt_draw_background(painter, tbrect.x(), tbrect.y(), tbrect.width(), h);
      ...
      //          // don't add leading to last line
      //          h += fm.leading();
      //        qt_draw_background(painter, r.x() + lb + ti.x(), r.y() + yoff + ti.y() - ti.ascent(), ti.width(), h);
      //      }
#endif
      //      painter->drawTextItem(r.x() + lb, r.y() + yoff, ti, textFlags);
      painter->drawTextItem(r.x() + lb, r.y() + yoff, ti, textFlags, tbrect);
      // ### AbanQ
    }
...
QRect QPainter::boundingRect(const QRect &r, int flags, const QString& str, int len, QTextParag **internal)
{
  QRect brect;
  if (str.isEmpty())
...
{
  return (p.data == data) || (p.data->linest == data->linest && p.data->width == data->width && p.data->color
      == data->color);
}
...
QDataStream &operator<<(QDataStream &s, const QPen &p)
{
  ...
  if (s.version() < 3)
    ...
    return s << (Q_UINT8) (p.style() | p.capStyle() | p.joinStyle()) << (Q_UINT8) p.width() << p.color();
}
...
{
  return (b.data == data) || (b.data->style == data->style && b.data->color == data->color && b.data->pixmap
      == data->pixmap);
}
