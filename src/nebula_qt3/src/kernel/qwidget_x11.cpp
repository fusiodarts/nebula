...
static uint aq_globalZ_ = 0;
static QWidget *aq_lastWidgetSetZ_ = 0;
static bool aq_notify_create_ = true;
extern void aq_notify_gui_event(QWidget *, QEvent *);
extern void aq_notify_gui_args_event(QWidget *, const QString &, const QStringList &);

static inline void aq_set_frame_data(QWidget *w, QTLWExtra *top)
{
  top->ftop = (w->testWFlags(Qt::WStyle_Title) ? 25 : 0);
...
void aq_update_frame(QWidget *w)
{
  QTLWExtra *top = w->topData();

  aq_set_frame_data(w, top);

  if (aq_lastWidgetSetZ_ != w) {
    aq_lastWidgetSetZ_ = w;
    aq_notify_gui_args_event(w, "zo", QStringList() << QString::number(++aq_globalZ_));
  }

  aq_notify_gui_args_event(w, "xfrm", QStringList() << QString::number(top->fleft) << QString::number(top->fright)
                           << QString::number(top->ftop) << QString::number(top->fbottom));

  if (!w->aq_frame_dirty_)
    w->fstrut_dirty = 0;
  else
    w->aq_frame_dirty_ = 0;

  //  qWarning("*** %s %d", w->className(), reinterpret_cast<Q_ULLONG> (w));
  //  qWarning("*** topData-> fleft:%d fright:%d ftop:%d fbottom:%d", top->fleft, top->fright, top->ftop, top->fbottom);
  //  qWarning("*** crect-> x:%d y:%d w:%d h:%d", w->crect.x(), w->crect.y(), w->crect.width(), w->crect.height());
}
// ### AbanQ
...
const uint stdWidgetEventMask = // X event mask
  (uint)(KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | KeymapStateMask | ButtonMotionMask
         | EnterWindowMask | LeaveWindowMask | FocusChangeMask | ExposureMask | PropertyChangeMask | StructureNotifyMask);

const uint stdDesktopEventMask = // X event mask
  (uint)(KeymapStateMask | EnterWindowMask | LeaveWindowMask | PropertyChangeMask);

/*
 The qt_ functions below are implemented in qwidgetcreate_x11.cpp.
 */

Window qt_XCreateWindow(const QWidget *creator, Display *display, Window parent, int x, int y, uint w, uint h,
                        int borderwidth, int depth, uint windowclass, Visual *visual, ulong valuemask,
                        XSetWindowAttributes *attributes);
Window qt_XCreateSimpleWindow(const QWidget *creator, Display *display, Window parent, int x, int y, uint w, uint h,
...
Q_EXPORT void qt_x11_enforce_cursor(QWidget *w)
{
  if (w->testWState(Qt::WState_OwnCursor)) {
    ...
    if (oc) {
      // ### AbanQ
      aq_notify_gui_args_event(w, "xcur", QString::number(oc->shape()));

      XDefineCursor(w->x11Display(), w->winId(), oc->handle());
    } else if (w->isEnabled()) {
      // ### AbanQ
      aq_notify_gui_args_event(w, "xcur", QString::number(w->cursor().shape()));

      XDefineCursor(w->x11Display(), w->winId(), w->cursor().handle());
    } else {
      // ### AbanQ
      aq_notify_gui_args_event(w, "xcur", QStringList());
...
static void qt_net_change_wm_state(const QWidget *w, bool set, Atom one, Atom two = 0)
{
  if (w->isShown()) {
    ...
    XSendEvent(w->x11Display(), RootWindow(w->x11Display(), w->x11Screen()), False, (SubstructureNotifyMask
    ...
    Atom *old_states = 0;
    status = XGetWindowProperty(w->x11Display(), w->winId(), qt_net_wm_state, 0, 1024, False, XA_ATOM, &ret, &format,
                                &nitems, &after, &data);
    ...
    if (j)
      XChangeProperty(w->x11Display(), w->winId(), qt_net_wm_state, XA_ATOM, 32, PropModeReplace, (uchar *) new_states,
                      j);
    else
...
void QWidget::create(WId window, bool initializeWindow, bool destroyOldWindow)
{
  ...
  if (desktop && qt_x11_create_desktop_on_screen >= 0 && qt_x11_create_desktop_on_screen != x11Screen()) {
  ...
  if (window) { // override the old window
    ...
    xd->x_defvisual = (XVisualIDFromVisual(a.visual) == XVisualIDFromVisual((Visual *) x11AppVisual(x11Screen())));
    ...
    if (x11DefaultVisual() && x11DefaultColormap()) {
      id = (WId) qt_XCreateSimpleWindow(this, dpy, parentw, crect.left(), crect.top(), crect.width(), crect.height(),
                                        0, black.pixel(x11Screen()), bg_col.pixel(x11Screen()));
    } else {
      wsa.background_pixel = bg_col.pixel(x11Screen());
      wsa.border_pixel = black.pixel(x11Screen());
      wsa.colormap = (Colormap) x11Colormap();
      id = (WId) qt_XCreateWindow(this, dpy, parentw, crect.left(), crect.top(), crect.width(), crect.height(), 0,
                                  x11Depth(), InputOutput, (Visual *) x11Visual(), CWBackPixel | CWBorderPixel
                                  | CWColormap, &wsa);
    }
    ...
    XChangeWindowAttributes(dpy, id, CWBackingStore, &wsa);
    if (aq_notify_create_)
      aq_notify_gui_event(this, new QEvent(QEvent::Create));
  }
  ...
  if (!initializeWindow) {
    ...
    wsa.override_redirect = True;
    wsa.save_under = True;
    XChangeWindowAttributes(dpy, id, CWOverrideRedirect | CWSaveUnder, &wsa);
    ...
    if (mwmhints.flags != 0l)
      XChangeProperty(dpy, id, qt_xa_motif_wm_hints, qt_xa_motif_wm_hints, 32, PropModeReplace,
    ...
    if (curr_wintype > 0)
      XChangeProperty(dpy, id, qt_net_wm_window_type, XA_ATOM, 32, PropModeReplace, (unsigned char *) net_wintypes,
                      curr_wintype);
    else
    ...
    if (curr_winstate > 0)
      XChangeProperty(dpy, id, qt_net_wm_state, XA_ATOM, 32, PropModeReplace, (unsigned char *) net_winstates,
                      curr_winstate);
    else
    ...
    XChangeProperty(dpy, id, qt_net_wm_pid, XA_CARDINAL, 32, PropModeReplace, (unsigned char *) &curr_pid, 1);

    // when we create a toplevel widget, the frame strut should be dirty
    fstrut_dirty = 1;

    // declare the widget's object name as window role
    XChangeProperty(dpy, id, qt_window_role, XA_STRING, 8, PropModeReplace, (unsigned char *) name(), qstrlen(name()));
...
void QWidget::reparentSys(QWidget *parent, WFlags f, const QPoint &p, bool showIt)
{
  ...
  XReparentWindow(x11Display(), old_winid, RootWindow(x11Display(), x11Screen()), 0, 0);
  ...
  // ### AbanQ
  aq_notify_create_ = false;
  create();
  aq_notify_create_ = true;
  // ### AbanQ
  ...
  if (chlist) { // reparent children
    ...
    while ((obj = it.current())) {
      if (obj->isWidgetType()) {
        ...
        if (!w->isTopLevel()) {
          XReparentWindow(x11Display(), w->winId(), winId(), w->geometry().x(), w->geometry().y());
        } else if (w->isPopup() || w->testWFlags(WStyle_DialogBorder) || w->testWFlags(WType_Dialog)
...
QPoint QWidget::mapToGlobal(const QPoint &pos) const
{
  int x, y;
  Window child;
  XTranslateCoordinates(x11Display(), winId(), QApplication::desktop()->screen(x11Screen())->winId(), pos.x(), pos.y(),
                        &x, &y, &child);
  return QPoint(x, y);
}
...
QPoint QWidget::mapFromGlobal(const QPoint &pos) const
{
  int x, y;
  Window child;
  XTranslateCoordinates(x11Display(), QApplication::desktop()->screen(x11Screen())->winId(), winId(), pos.x(), pos.y(),
                        &x, &y, &child);
  return QPoint(x, y);
}
...
void QWidget::setBackgroundColorDirect(const QColor &color)
{
  ...
  // ### AbanQ
  aq_notify_gui_args_event(this, "xsbg", bg_col.name());
...
qstring_to_xtp(const QString &s)
{
  ...
  if (mapper) {
    ...
    tl[0] = mapped.data();
    tl[1] = 0;
    errCode = XmbTextListToTextProperty(QPaintDevice::x11AppDisplay(), tl, 1, XStdICCTextStyle, &tp);
#if defined(QT_DEBUG)
    if (errCode < 0)
...
void QWidget::setCaption(const QString &caption)
{
  ...
  XChangeProperty(x11Display(), winId(), qt_net_wm_name, qt_utf8_string, 8, PropModeReplace,
                  (unsigned char *) net_wm_name.data(), net_wm_name.length());
...
void QWidget::setMouseTracking(bool enable)
{
  ...
  if (testWFlags(WType_Desktop)) { // desktop widget?
    ...
    if (main_desktop->testWFlags(WPaintDesktop))
      XSelectInput(x11Display(), winId(), stdDesktopEventMask | ExposureMask);
    else
      XSelectInput(x11Display(), winId(), stdDesktopEventMask);
  } else {

    // ### AbanQ
    aq_notify_gui_args_event(this, "xsmt", (enable ? "1" : "0"));

    XSelectInput(x11Display(), winId(), m | stdWidgetEventMask);
#if defined (QT_TABLET_SUPPORT)
    if (devStylus != NULL) {
...
void QWidget::grabMouse()
{
  if (isVisible() && !qt_nograb()) {
    ...
    int status =
#endif
      XGrabPointer(x11Display(), winId(), False, (uint)(ButtonPressMask | ButtonReleaseMask | PointerMotionMask
                                                        | EnterWindowMask | LeaveWindowMask), GrabModeAsync, GrabModeAsync, None, None, qt_x_time);
#if defined(QT_CHECK_STATE)
    if (status) {
      const char *s = status == GrabNotViewable ? "\"GrabNotViewable\""
                      : status == AlreadyGrabbed ? "\"AlreadyGrabbed\"" : status
...
#endif
      XGrabPointer(x11Display(), winId(), False, (uint)(ButtonPressMask | ButtonReleaseMask | PointerMotionMask
                                                        | EnterWindowMask | LeaveWindowMask), GrabModeAsync, GrabModeAsync, None, cursor.handle(), qt_x_time);
#if defined(QT_CHECK_STATE)
    if (status) {
      const char *s = status == GrabNotViewable ? "\"GrabNotViewable\""
                      : status == AlreadyGrabbed ? "\"AlreadyGrabbed\"" : status
...
#ifdef AQ_WID_PM
        {
          QPainter pterpm(aq_pm());
          pterpm.fillRect(x, y, w, h, eraseColor());
        }
#endif
        if (w > 1 && h > 1)
          aq_notify_gui_args_event(this, "xcla", QStringList() << AQ_STR_NUMBER_COAL(x) << AQ_STR_NUMBER_COAL(y)
                                   << AQ_STR_NUMBER_COAL(w) << AQ_STR_NUMBER_COAL(h));
  ...
  if (isTopLevel()) {
    ...
    if ((oldstate & WindowMaximized) != (newstate & WindowMaximized)) {
      if (qt_net_supports(qt_net_wm_state_max_h) && qt_net_supports(qt_net_wm_state_max_v)) {
        qt_net_change_wm_state(this, (newstate & WindowMaximized), qt_net_wm_state_max_h, qt_net_wm_state_max_v);
        ...
        if (newstate & WindowMaximized) {
          ...
          if (isVisible()) {
            ...
            // ### AbanQ
            aq_set_frame_data(this, top);

            const QRect maxRect = QApplication::desktop()->availableGeometry(this);
            const QRect r = top->normalGeometry;
            setGeometry(maxRect.x() + top->fleft, maxRect.y() + top->ftop, maxRect.width() - top->fleft - top->fright,
    ...
    if ((oldstate & WindowFullScreen) != (newstate & WindowFullScreen)) {
      if (qt_net_supports(qt_net_wm_state_fullscreen)) {
        qt_net_change_wm_state(this, (newstate & WindowFullScreen), qt_net_wm_state_fullscreen);
      } else {
        needShow = isVisible();
        ...
        if (newstate & WindowFullScreen) {
          ...
          reparent(0, WType_TopLevel | WStyle_Customize | WStyle_NoBorder |
                   ...
                   (getWFlags() & 0xffff0000), mapToGlobal(QPoint(0, 0)));
          const QRect r = top->normalGeometry;
    ...
    if ((oldstate & WindowMinimized) != (newstate & WindowMinimized)) {
      if (isVisible()) {
        if (newstate & WindowMinimized) {
          ...
          e.xclient.data.l[3] = 0;
          e.xclient.data.l[4] = 0;
          XSendEvent(x11Display(), RootWindow(x11Display(), x11Screen()), False, (SubstructureNotifyMask
                     | SubstructureRedirectMask), &e);
        } else {
...
void QWidget::showWindow()
{
  if (isTopLevel()) {
    ...
    if (qt_x_user_time != CurrentTime) {
      XChangeProperty(x11Display(), winId(), qt_net_wm_user_time, XA_CARDINAL, 32, PropModeReplace,
                      (unsigned char *) &qt_x_user_time, 1);
    }

    if (!topData()->embedded && topData()->parentWinId && topData()->parentWinId
        != QPaintDevice::x11AppRootWindow(x11Screen()) && !isMinimized()) {
      qt_deferred_map_add(this);
      return;
    }

    if (isMaximized() && !isFullScreen() && !(qt_net_supports(qt_net_wm_state_max_h)
...
void QWidget::raise()
{
  ...
  // ### AbanQ
  if (aq_lastWidgetSetZ_ != this) {
    aq_lastWidgetSetZ_ = this;
    aq_notify_gui_args_event(this, "zo", QString::number(++aq_globalZ_));
  }
...
void QWidget::lower()
{
  ...
  // ### AbanQ
  aq_notify_gui_args_event(this, "zo", "0");
...
static void do_size_hints(QWidget *widget, QWExtra *x)
{
  ...
  if (x) {
    ...
    if (x->topextra && (x->topextra->incw > 0 || x->topextra->inch > 0)) { // add resize increment hints
  ...
  if (isVisible()) {
    ...
    if (isMove && pos() != oldPos) {
      if (!qt_broken_wm) {
        ...
        // work around 4Dwm's incompliance with ICCCM 4.1.5
        QApplication::postEvent(this, new QMoveEvent(crect.topLeft(), oldGeom.topLeft()));
      }
    }
    if (isResize)
...
void QWidget::setMaximumSize(int maxw, int maxh)
{
  ...
  if (maxw > QWIDGETSIZE_MAX || maxh > QWIDGETSIZE_MAX) {
    qWarning("QWidget::setMaximumSize: (%s/%s) "
             "The largest allowed size is (%d,%d)", name("unnamed"), className(), QWIDGETSIZE_MAX, QWIDGETSIZE_MAX);
    maxw = QMIN(maxw, QWIDGETSIZE_MAX);
    maxh = QMIN(maxh, QWIDGETSIZE_MAX);
  }
  if (maxw < 0 || maxh < 0) {
    qWarning("QWidget::setMaximumSize: (%s/%s) Negative sizes (%d,%d) "
             "are not possible", name("unnamed"), className(), maxw, maxh);
    maxw = QMAX(maxw, 0);
    maxh = QMAX(maxh, 0);
  }
#endif
...
void QWidget::scroll(int dx, int dy, const QRect &r)
{
  ...
  if (w > 0 && h > 0 && !just_update) {
    ...
    QPixmap *wpm = aq_pm();
    wpm->detach();
    XCopyArea(dpy, wpm->handle(), wpm->handle(), gc, x1, y1, w, h, x2, y2);
#endif
    aq_notify_gui_args_event(this, "xsll", QStringList() << QString::number(dx) << QString::number(dy));
  ...
  if (dx) {
    ...
    if (repaint_immediately)
      ...
      //      {
      //        QPainter pterpm(aq_pm());
      //        pterpm.fillRect(x, sr.y(), QABS(dx), sr.height(), eraseColor());
      //      }
      //#endif
      //      aq_notify_gui_args_event(this, "xcla", QStringList() << QString::number(x) << QString::number(sr.y())
      //          << QString::number(QABS(dx)) << QString::number(sr.height()));
  ...
  if (dy) {
    ...
    if (repaint_immediately)
      ...
      //      {
      //        QPainter pterpm(aq_pm());
      //        pterpm.fillRect(sr.x(), y, sr.width(), QABS(dy), eraseColor());
      //      }
      //#endif
      //      aq_notify_gui_args_event(this, "xcla", QStringList() << QString::number(sr.x()) << QString::number(y)
      //          << QString::number(sr.width()) << QString::number(QABS(dy)));
  ...
  if (m == QPaintDeviceMetrics::PdmWidth) {
    ...
    switch (m) {
      ...
      case QPaintDeviceMetrics::PdmWidthMM:
        val = (DisplayWidthMM(dpy, scr) * crect.width()) / DisplayWidth(dpy, scr);
        break;
      case QPaintDeviceMetrics::PdmHeightMM:
        val = (DisplayHeightMM(dpy, scr) * crect.height()) / DisplayHeight(dpy, scr);
        break;
    ...
    if (children) {
      ...
      while ((object = it.current())) {
        ...
        if (object->isWidgetType()) {
          ...
          children_use_dnd = (children_use_dnd || child->acceptDrops() || (child->extra
...
{
  XShapeCombineRegion(x11Display(), winId(), ShapeBounding, 0, 0, region.handle(), ShapeSet);
}
...
{
  XShapeCombineMask(x11Display(), winId(), ShapeBounding, 0, 0, None, ShapeSet);
}
...
void QWidget::setName(const char *name)
{
  ...
  if (isTopLevel()) {
    XChangeProperty(x11Display(), winId(), qt_window_role, XA_STRING, 8, PropModeReplace, (unsigned char *) name,
                    qstrlen(name));
  }
}
...
void QWidget::updateFrameStrut() const
{
  ...
  while (XQueryTree(QPaintDevice::x11AppDisplay(), w, &r, &p, &c, &nc)) {
    ...
    data_ret = 0;
    if (p == r || (XGetWindowProperty(QPaintDevice::x11AppDisplay(), p, qt_enlightenment_desktop, 0, 1, False,
                                      XA_CARDINAL, &type_ret, &i_unused, &l_unused, &l_unused, &data_ret) == Success
  ...
  if (XTranslateCoordinates(QPaintDevice::x11AppDisplay(), l, w, 0, 0, &transx, &transy, &p)
