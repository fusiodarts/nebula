...
// ### AbanQ
extern const QRect& aq_desktop_geometry();
...
QDesktopWidgetPrivate::QDesktopWidgetPrivate() :
  use_xinerama(FALSE), defaultScreen(0), screenCount(1), screens(0), rects(0), workareas(0)
{
}
...
void QDesktopWidgetPrivate::init()
{
  ...
  int unused;
  use_xinerama = (XineramaQueryExtension(QPaintDevice::x11AppDisplay(), &unused, &unused)
      && XineramaIsActive(QPaintDevice::x11AppDisplay()));

  if (use_xinerama) {
    xinerama_screeninfo = XineramaQueryScreens(QPaintDevice::x11AppDisplay(), &screenCount);
    defaultScreen = 0;
  } else
