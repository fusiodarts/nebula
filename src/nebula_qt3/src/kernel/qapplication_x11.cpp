...
static int qt_x_errhandler(Display *dpy, XErrorEvent *err)
{
  ...
  qWarning("X Error: %s %d\n"
    "  Major opcode:  %d\n"
    "  Minor opcode:  %d\n"
    "  Resource id:  0x%lx", errstr, err->error_code, err->request_code, err->minor_code, err->resourceid);
...
bool QApplication::x11_apply_settings()
{
  ...
  if (XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(0), qt_settings_timestamp, 0, 0, False,
                         AnyPropertyType, &type, &format, &nitems, &after, &data) == Success && format == 8) {
    if (data)
      XFree(data);
    ...
    while (after > 0) {
      XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(0), qt_settings_timestamp, offset, 1024, False,
  ...
  if (strlist.count() == QColorGroup::NColorRoles) {
    for (i = 0; i < QColorGroup::NColorRoles; i++)
      pal.setColor(QPalette::Active, (QColorGroup::ColorRole) i, QColor(strlist[i]));
  }
  ...
  if (pal.disabled().buttonText() == pal.active().buttonText()) {
    pal.setColor(QPalette::Disabled, QColorGroup::ButtonText, pal.disabled().foreground());
  }
  ...
  QString libpathkey = QString("/qt/%1.%2/libraryPath").arg(QT_VERSION >> 16).arg((QT_VERSION & 0xff00) >> 8);
  ...
  qt_broken_wm = settings.readBoolEntry("/qt/brokenWindowManager", qt_broken_wm);

  qt_resolve_symlinks = settings.readBoolEntry("/qt/resolveSymlinks", TRUE);

  qt_use_rtl_extensions = settings.readBoolEntry("/qt/useRtlExtensions", FALSE);
  ...
  if (update_timestamp) {
    ...
    XChangeProperty(appDpy, QPaintDevice::x11AppRootWindow(0), qt_settings_timestamp, qt_settings_timestamp, 8,
                    PropModeReplace, (unsigned char *) stamp.buffer().data(), stamp.buffer().size());
  }
...
static void qt_set_input_encoding()
{
  ...
  int e = XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_input_encoding, 0, 1024, False, XA_STRING,
...
static void qt_set_x11_resources(const char* font = 0, const char* fg = 0, const char* bg = 0, const char* button = 0)
{
  ...
  if (QApplication::desktopSettingsAware() && !QApplication::x11_apply_settings()) {
    ...
    while (after > 0) {
      uchar *data;
      XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(0), qt_resource_manager, offset, 8192, False,
                         AnyPropertyType, &type, &format, &nitems, &after, &data);
      res += (char*) data;
    ...
    while (l < resl) {
      ...
      while (isspace((uchar) res[l]))
        l++;
      bool mine = FALSE;
      if (res[l] == '*' && (res[l + 1] == 'f' || res[l + 1] == 'b' || res[l + 1] == 'g' || res[l + 1] == 'F' || res[l
          + 1] == 'B' || res[l + 1] == 'G' || res[l + 1] == 's' || res[l + 1] == 'S')) {
    ...
    if (!fnt.exactMatch()) {
      ...
      if (!fnt.rawMode()) {
        ...
        if (fnt.pointSize() <= 0 && fnt.pixelSize() <= 0)
          ...
          fnt.setPointSize((int) ((fontinfo.pixelSize() * 72. / (float) QPaintDevice::x11AppDpiY()) + 0.5));
      }
    }
  ...
  if (button || !resBG.isEmpty() || !resFG.isEmpty()) {// set app colors
    ...
    QColorGroup cg(fg, btn, btn.light(), btn.dark(), btn.dark(150), fg, Qt::white, base, bg);
    if (bright_mode) {
    ...
    QColor disabled((fg.red() + btn.red()) / 2, (fg.green() + btn.green()) / 2, (fg.blue() + btn.blue()) / 2);
    QColorGroup dcg(disabled, btn, btn.light(125), btn.dark(), btn.dark(150), disabled, Qt::white, Qt::white, bg);
    if (bright_mode) {
...
static void qt_detect_broken_window_manager()
{
  ...
  // look for SGI's 4Dwm
  int e = XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_sgi_desks_manager, 0, 1, False, XA_WINDOW,
...
void qt_get_net_supported()
{
  ...
  int e = XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_net_supported, 0, 0, False, XA_ATOM, &type,
  ...
  if (e == Success && type == XA_ATOM && format == 32) {
    ...
    while (after > 0) {
      XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_net_supported, offset, 1024, False, XA_ATOM,
                         &type, &format, &nitems, &after, &data);
...
void qt_get_net_virtual_roots()
{
  ...
  int e = XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_net_virtual_roots, 0, 0, False, XA_ATOM,
                             &type, &format, &nitems, &after, &data);
  if (data)
    XFree(data);
  ...
  if (e == Success && type == XA_ATOM && format == 32) {
    ...
    while (after > 0) {
      XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_net_virtual_roots, offset, 1024, False, XA_ATOM,
                         &type, &format, &nitems, &after, &data);
...
void qt_x11_create_wm_client_leader()
{
  ...
  qt_x11_wm_client_leader = XCreateSimpleWindow(QPaintDevice::x11AppDisplay(), QPaintDevice::x11AppRootWindow(), 0, 0,
                                                1, 1, 0, 0, 0);

  // set client leader property to itself
  XChangeProperty(QPaintDevice::x11AppDisplay(), qt_x11_wm_client_leader, qt_wm_client_leader, XA_WINDOW, 32,
                  PropModeReplace, (unsigned char *) &qt_x11_wm_client_leader, 1);

  // If we are session managed, inform the window manager about it
  QCString session = qApp->sessionId().latin1();
  if (!session.isEmpty()) {
    XChangeProperty(QPaintDevice::x11AppDisplay(), qt_x11_wm_client_leader, qt_sm_client_id, XA_STRING, 8,
                    PropModeReplace, (unsigned char *) session.data(), session.length());
  }
}

static void qt_net_update_user_time(QWidget *tlw)
{
  XChangeProperty(QPaintDevice::x11AppDisplay(), tlw->winId(), qt_net_wm_user_time, XA_CARDINAL, 32, PropModeReplace,
...
static Visual *find_truecolor_visual(Display *dpy, int scr, int *depth, int *ncols)
{
  ...
  rvi.screen = scr;
  vi = XGetVisualInfo(dpy, VisualClassMask | VisualScreenMask, &rvi, &n);
  if (vi) {
    for (i = 0; i < n; i++) {
  ...
  if (!vi || (vi[best].visualid == XVisualIDFromVisual(v)) || (vi[best].depth <= 8 && qt_visual_option != TrueColor)) {
...
void qt_init_internal(int *argcptr, char **argv, Display *display, Qt::HANDLE visual, Qt::HANDLE colormap)
{
    ...
    *argcptr = j;
    ...
    if (qt_is_gui_used) {
      if ((appDpy = XOpenDisplay(appDpyName)) == 0) {
        qWarning("%s: cannot connect to X server %s", appName, XDisplayName(appDpyName));
        qApp = 0;
        exit(1);
      }
  ...
  if (qt_is_gui_used) {
    ...
    for (screen = 0; screen < appScreenCount; ++screen) {
      ...
      if (!vis) {
        ...
        if (qt_visual_option == TrueColor || QApplication::colorSpec() == QApplication::ManyColor) {
          ...
          QPaintDevice::x_appdefvisual_arr[screen] = (XVisualIDFromVisual(vis)
      ...
      if (vis->c_class == TrueColor && QPaintDevice::x_appdepth_arr[screen] == 8
      ...
      if (colormap && screen == appScreen) {
        ...
        if (vis->c_class == TrueColor) {
          QPaintDevice::x_appdefcolormap_arr[screen] = QPaintDevice::x_appdefvisual_arr[screen];
        } else {
          QPaintDevice::x_appdefcolormap_arr[screen] = !qt_cmap_option && QPaintDevice::x_appdefvisual_arr[screen];
        }

        if (QPaintDevice::x_appdefcolormap_arr[screen]) {
          // use default colormap
          XStandardColormap *stdcmap;
          VisualID vid = XVisualIDFromVisual((Visual *) QPaintDevice::x_appvisual_arr[screen]);
          int i, count;
          ...
          if (!serverVendor.contains("Hewlett-Packard")) {
            ...
            if (XGetRGBColormaps(appDpy, QPaintDevice::x11AppRootWindow(screen), &stdcmap, &count, XA_RGB_DEFAULT_MAP)) {
              i = 0;
              while (i < count && QPaintDevice::x_appcolormap_arr[screen] == 0) {
                if (stdcmap[i].visualid == vid) {
                  QPaintDevice::x_appcolormap_arr[screen] = stdcmap[i].colormap;
                }
                i++;
              }
          ...
          if (QPaintDevice::x_appcolormap_arr[screen] == 0) {
            QPaintDevice::x_appcolormap_arr[screen] = DefaultColormap(appDpy, screen);
          }
        } else {
          // create a custom colormap
          QPaintDevice::x_appcolormap_arr[screen] = XCreateColormap(appDpy, QPaintDevice::x11AppRootWindow(screen),
                                                                    vis, AllocNone);
        }
      }
    }
    ...
    qt_x11_intern_atom("_NET_WM_STATE_STAYS_ON_TOP", &qt_net_wm_state_stays_on_top);
    ...
    if (XRenderQueryExtension(appDpy, &xrender_eventbase, &xrender_errorbase)) {
      ...
      // default visual
      XRenderPictFormat *format = XRenderFindVisualFormat(appDpy, (Visual *) QPaintDevice::x_appvisual);
    ...
    if (map) {
      ...
      for (maskIndex = 0; maskIndex < 8; maskIndex++) {
        for (i = 0; i < map->max_keypermod; i++) {
          if (map->modifiermap[mapIndex]) {
            ...
            if (qt_alt_mask == 0 && (sym == XK_Alt_L || sym == XK_Alt_R)) {
              qt_alt_mask = 1 << maskIndex;
            }
            if (qt_meta_mask == 0 && (sym == XK_Meta_L || sym == XK_Meta_R)) {
      ...
      mapIndex = 0;
      for (maskIndex = 0; maskIndex < 8; maskIndex++) {
        if (qt_alt_mask != (1 << maskIndex) && qt_meta_mask != (1 << maskIndex)) {
          for (i = 0; i < map->max_keypermod; i++)
            mapIndex++;
          continue;
        }
  ...
  if (qt_is_gui_used) {
    ...
    int screen;
    for (screen = 0; screen < appScreenCount; ++screen) {
      XSelectInput(appDpy, QPaintDevice::x11AppRootWindow(screen), KeymapStateMask | EnterWindowMask | LeaveWindowMask
          | PropertyChangeMask);
  ...
  if (qt_is_gui_used) {
    ...
    int ptsz =
        (int) (((QPaintDevice::x11AppDpiY() >= 95 ? 17. : 12.) * 72. / (float) QPaintDevice::x11AppDpiY()) + 0.5);
...
void QApplication::x11_initialize_style()
{
  ...
  uchar *data;
  if (!app_style && XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_kwin_running, 0, 1, False,
                                       AnyPropertyType, &type, &format, &length, &after, &data) == Success && length) {
    if (data)
      XFree((char *) data);
  ...
  if (!app_style && XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_kwm_running, 0, 1, False,
                                       AnyPropertyType, &type, &format, &length, &after, &data) == Success && length) {
    if (data)
      XFree((char *) data);
    app_style = QStyleFactory::create("windows");
  }
  if (!app_style && XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_cde_running, 0, 1, False,
                                       AnyPropertyType, &type, &format, &length, &after, &data) == Success && length) {
  ...
  if (!app_style && XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_gbackground_properties, 0, 1,
                                       False, AnyPropertyType, &type, &format, &length, &after, &data) == Success
      && length) {
    if (data)
      XFree((char *) data);
  ...
  if (qt_is_gui_used) {
    ...
    for (screen = 0; screen < appScreenCount; screen++) {
      if (!QPaintDevice::x11AppDefaultColormap(screen))
        XFreeColormap(QPaintDevice::x11AppDisplay(), QPaintDevice::x11AppColormap(screen));
    }
  }
...
void qt_save_rootinfo() // save new root info
{
  ...
  if (qt_xsetroot_id) { // kill old pixmap
    if (XGetWindowProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_xsetroot_id, 0, 1, True, AnyPropertyType,
                           &type, &format, &length, &after, &data) == Success) {
      if (type == XA_PIXMAP && format == 32 && length == 1 && after == 0 && data) {
        XKillClient(appDpy, *((Pixmap*) data));
      }
      Pixmap dummy = XCreatePixmap(appDpy, QPaintDevice::x11AppRootWindow(), 1, 1, 1);
      XChangeProperty(appDpy, QPaintDevice::x11AppRootWindow(), qt_xsetroot_id, XA_PIXMAP, 32, PropModeReplace,
                      (uchar *) &dummy, 1);
...
bool qt_wstate_iconified(WId winid)
{
  ...
  unsigned long length, after;
  uchar *data;
  int r = XGetWindowProperty(appDpy, winid, qt_wm_state, 0, 2, False, AnyPropertyType, &type, &format, &length, &after,
                             &data);
  bool iconic = FALSE;
...
static GC create_gc(int scrn, bool monochrome)
{
  ...
  if (monochrome) {
    ...
    if (QPaintDevice::x11AppDefaultVisual(scrn)) {
      ...
      w = XCreateWindow(appDpy, RootWindow(appDpy, scrn), 0, 0, 100, 100, 0, QPaintDevice::x11AppDepth(scrn),
                        InputOutput, (Visual*) QPaintDevice::x11AppVisual(scrn), CWBackPixel | CWBorderPixel
                            | CWColormap, &a);
...
void QApplication::setMainWidget(QWidget *mainWidget)
{
  ...
  if (main_widget) { // give WM command line
    XSetWMProperties(main_widget->x11Display(), main_widget->winId(), 0, 0, app_argv, app_argc, 0, 0, 0);
    if (mwTitle)
...
Window qt_x11_findClientWindow(Window win, Atom property, bool leaf)
{
  ...
  uint nchildren;
  if (XGetWindowProperty(appDpy, win, property, 0, 0, FALSE, AnyPropertyType, &type, &format, &nitems, &after, &data)
      == Success) {
    if (data)
      XFree((char *) data);
    if (type)
...
QWidget *QApplication::widgetAt(int x, int y, bool child)
{
  ...
  Window target;
  if (!XTranslateCoordinates(appDpy, QPaintDevice::x11AppRootWindow(screen), QPaintDevice::x11AppRootWindow(screen), x,
                             y, &lx, &ly, &target)) {
    return 0;
  }
    ...
    if (event->xclient.message_type == qt_wm_protocols) {
      ...
      if (a == qt_wm_delete_window) {
        ...
        if (amw && amw != widget) {
          ...
          while (groupLeader && !groupLeader->testWFlags(Qt::WGroupLeader) && groupLeader != amw)
        ...
        if (event->xclient.window != root) {
          ...
          XSendEvent(event->xclient.display, event->xclient.window, False, SubstructureNotifyMask
...
int QApplication::x11ProcessEvent(XEvent* event)
{
  ...
  switch (event->type) {
    ...
    case ButtonPress:
      if (event->xbutton.root != RootWindow(widget->x11Display(), widget->x11Screen()) && !qt_xdnd_dragging) {
    ...
    case XFocusIn: { // got focus
      ...
      if (!widget->isTopLevel())
        break;
      if (event->xfocus.detail != NotifyAncestor && event->xfocus.detail != NotifyInferior && event->xfocus.detail
          != NotifyNonlinear)
        break;
    ...
    case XFocusOut: // lost focus
      ...
      if (event->xfocus.mode != NotifyNormal)
        break;
      if (event->xfocus.detail != NotifyAncestor && event->xfocus.detail != NotifyNonlinearVirtual
    ...
    case EnterNotify: { // enter window
      ...
      if (inPopupMode() && widget->topLevelWidget() != activePopupWidget())
        break;
      if (event->xcrossing.mode != NotifyNormal || event->xcrossing.detail == NotifyVirtual || event->xcrossing.detail
          == NotifyNonlinearVirtual)
        break;
      if (event->xcrossing.focus && !widget->isDesktop() && !widget->isActiveWindow()) {
    ...
    case LeaveNotify: { // leave window
      ...
      QWidget* enter = 0;
      XEvent ev;
      while (XCheckMaskEvent(widget->x11Display(), EnterWindowMask | LeaveWindowMask, &ev) && !qt_x11EventFilter(&ev)) {
        ...
        if (ev.xcrossing.mode != NotifyNormal || ev.xcrossing.detail == NotifyVirtual || ev.xcrossing.detail
            == NotifyNonlinearVirtual)
          continue;
        enter = event_widget;
        if (ev.xcrossing.focus && enter && !enter->isDesktop() && !enter->isActiveWindow()) {
          if (qt_focus_model == FocusModel_Unknown) // check focus model
            qt_check_focus_model();
          if (qt_focus_model == FocusModel_PointerRoot) // PointerRoot mode
      ...
      if ((!enter || enter->isDesktop()) && event->xcrossing.focus && widget == active_window && qt_focus_model
          == FocusModel_PointerRoot // PointerRoot mode
      ) {
        setActiveWindow(0);
      }
    ...
    case MapNotify: // window shown
      if (widget->isTopLevel() && widget->topData()->spont_unmapped) {
    ...
    case ReparentNotify: // window manager reparents
      while (XCheckTypedWindowEvent(widget->x11Display(), widget->winId(), ReparentNotify, event))
      ...
      if (widget->isTopLevel()) {
        ...
        widget->topData()->fleft = widget->topData()->fright = widget->topData()->ftop = widget->topData()->fbottom = 0;
...
void QApplication::openPopup(QWidget *popup)
{
  ...
  if (popupWidgets->count() == 1 && !qt_nograb()) { // grab mouse/keyboard
    int r = XGrabKeyboard(popup->x11Display(), popup->winId(), FALSE, GrabModeSync, GrabModeAsync, CurrentTime);
    if ((popupGrabOk = (r == GrabSuccess))) {
      r = XGrabPointer(popup->x11Display(), popup->winId(), TRUE, (uint) (ButtonPressMask | ButtonReleaseMask
          | ButtonMotionMask | EnterWindowMask | LeaveWindowMask | PointerMotionMask), GrabModeSync, GrabModeAsync,
...
void QApplication::closePopup(QWidget *popup)
{
  ...
  if (popupWidgets->count() == 0) { // this was the last popup
    ...
    if (!qt_nograb() && popupGrabOk) { // grabbing not disabled
      if (mouseButtonState != 0 || popup->geometry(). contains(QPoint(mouseGlobalXPos, mouseGlobalYPos))) { // mouse release event or inside
    ...
    QFocusEvent::resetReason();
    if (popupWidgets->count() == 1 && !qt_nograb()) { // grab mouse/keyboard
      int r = XGrabKeyboard(aw->x11Display(), aw->winId(), FALSE, GrabModeSync, GrabModeAsync, CurrentTime);
      if ((popupGrabOk = (r == GrabSuccess))) {
        r = XGrabPointer(aw->x11Display(), aw->winId(), TRUE, (uint) (ButtonPressMask | ButtonReleaseMask
            | ButtonMotionMask | EnterWindowMask | LeaveWindowMask | PointerMotionMask), GrabModeSync, GrabModeAsync,
                         None, None, CurrentTime);
...
bool QETWidget::translateMouseEvent(const XEvent *event)
{
  ...
  if (event->type == MotionNotify) { // mouse move
    if (event->xmotion.root != RootWindow(appDpy, x11Screen()) && !qt_xdnd_dragging)
      return FALSE;

    XMotionEvent lastMotion = event->xmotion;
    while (XPending(appDpy)) { // compres mouse moves
      XNextEvent(appDpy, &nextEvent);
      if (nextEvent.type == ConfigureNotify || nextEvent.type == PropertyNotify || nextEvent.type == Expose
          || nextEvent.type == NoExpose) {
        qApp->x11ProcessEvent(&nextEvent);
        continue;
      } else if (nextEvent.type != MotionNotify || nextEvent.xmotion.window != event->xmotion.window
          || nextEvent.xmotion.state != event->xmotion.state) {
        XPutBackEvent(appDpy, &nextEvent);
        break;
      }
      if (!qt_x11EventFilter(&nextEvent) && !x11Event(&nextEvent)) // send event through filter
    ...
    if (qt_button_down && (state & (LeftButton | MidButton | RightButton)) == 0)
      qt_button_down = 0;

    // throw away mouse move events that are sent multiple times to the same
    // position
    bool throw_away = FALSE;
    if (x_root_save == globalPos.x() && y_root_save == globalPos.y())
      throw_away = TRUE;
    ...
    switch (event->xbutton.button) {
      ...
      case 7:
        ...
        if (event->type == ButtonPress) {
          ...
          int delta = 1;
          XEvent xevent;
          while (XCheckTypedWindowEvent(x11Display(), winId(), ButtonPress, &xevent)) {
          ...
          bool hor = ((btn == Button4 || btn == Button5) && (state & AltButton) || (btn == 6 || btn == 7));
    ...
    if (event->type == ButtonPress) { // mouse button pressed
      ...
      if (!qt_button_down || !qt_button_down->testWFlags(WMouseNoMask))
        qt_button_down = this;
      if (mouseActWindow == event->xbutton.window && mouseButtonPressed == button && (long) event->xbutton.time
          - (long) mouseButtonPressTime < QApplication::doubleClickInterval() && QABS(event->xbutton.x - mouseXPos) < 5
  ...
  if (qApp->inPopupMode()) { // in popup mode
    ...
    if (popupButtonFocus) {
      QMouseEvent e(type, popupButtonFocus->mapFromGlobal(globalPos), globalPos, button, state);
      ...
      QMouseEvent e(type, popupChild->mapFromGlobal(globalPos), globalPos, button, state);
    ...
    if (qApp->inPopupMode()) { // still in popup mode
      ...
      if (type != QEvent::MouseButtonRelease && state != 0 && QWidget::find((WId) mouseActWindow)) {
        manualGrab = TRUE; // need to manually grab
        XGrabPointer(dpy, mouseActWindow, False, (uint) (ButtonPressMask | ButtonReleaseMask | ButtonMotionMask
            | EnterWindowMask | LeaveWindowMask), GrabModeAsync, GrabModeAsync, None, None, CurrentTime);
      }
    }
    ...
    if (type == QEvent::MouseButtonRelease && (state & (~button) & (LeftButton | MidButton | RightButton)) == 0) {
      qt_button_down = 0;
    }
...
bool QETWidget::translateWheelEvent(int global_x, int global_y, int delta, int state, Orientation orient)
{
  // send the event to the widget or its ancestors
  {
    ...
    if (popup && topLevelWidget() != popup)
      popup->close();
    QWheelEvent e(mapFromGlobal(QPoint(global_x, global_y)), QPoint(global_x, global_y), delta, state, orient);
...
bool QETWidget::translatePropertyEvent(const XEvent *event)
{
  ...
  if (event->xproperty.atom == qt_net_wm_frame_strut) {
    ...
    if (event->xproperty.state == PropertyNewValue) {
      e = XGetWindowProperty(appDpy, event->xproperty.window, qt_net_wm_frame_strut, 0, 4, // struts are 4 longs
    ...
    if (event->xproperty.state == PropertyNewValue) {
      ...
      // possible NET states...
      e = XGetWindowProperty(appDpy, event->xproperty.window, qt_net_wm_state, 0, 1024, False, XA_ATOM, &ret, &format,
                             &nitems, &after, &data);
    ...
    if (qt_net_supports(qt_net_wm_state_max_v) && qt_net_supports(qt_net_wm_state_max_h)) {
      if (max && !isMaximized()) {
    ...
    if (event->xproperty.state == PropertyDelete) {
      ...
      e = XGetWindowProperty(appDpy, winId(), qt_wm_state, 0, 2, False, qt_wm_state, &ret, &format, &nitems, &after,
                             &data);
...
static const KeySym KeyTbl[] = { // keyboard mapping table
        ...
        // browsing keys
        XF86XK_Back, Qt::Key_Back, XF86XK_Forward, Qt::Key_Forward, XF86XK_Stop, Qt::Key_Stop, XF86XK_Refresh,
        Qt::Key_Refresh, XF86XK_Favorites, Qt::Key_Favorites, XF86XK_AudioMedia,
        Qt::Key_LaunchMedia,
        XF86XK_OpenURL,
        Qt::Key_OpenUrl,
        XF86XK_HomePage,
        ...
        // media keys
        XF86XK_AudioLowerVolume, Qt::Key_VolumeDown, XF86XK_AudioMute, Qt::Key_VolumeMute, XF86XK_AudioRaiseVolume,
        Qt::Key_VolumeUp, XF86XK_AudioPlay, Qt::Key_MediaPlay, XF86XK_AudioStop, Qt::Key_MediaStop, XF86XK_AudioPrev,
        Qt::Key_MediaPrev,
        XF86XK_AudioNext,
        Qt::Key_MediaNext,
        XF86XK_AudioRecord,
        ...
        // launch keys
        XF86XK_Mail, Qt::Key_LaunchMail, XF86XK_MyComputer, Qt::Key_Launch0, XF86XK_Calculator, Qt::Key_Launch1,
        XF86XK_Standby, Qt::Key_Standby,

        XF86XK_Launch0, Qt::Key_Launch2, XF86XK_Launch1, Qt::Key_Launch3, XF86XK_Launch2, Qt::Key_Launch4,
        XF86XK_Launch3, Qt::Key_Launch5, XF86XK_Launch4, Qt::Key_Launch6, XF86XK_Launch5, Qt::Key_Launch7,
        XF86XK_Launch6, Qt::Key_Launch8, XF86XK_Launch7, Qt::Key_Launch9, XF86XK_Launch8, Qt::Key_LaunchA,
        XF86XK_Launch9, Qt::Key_LaunchB, XF86XK_LaunchA, Qt::Key_LaunchC, XF86XK_LaunchB, Qt::Key_LaunchD,
...
bool QETWidget::translateKeyEventInternal(const XEvent *event, int& count, QString& text, int& state, char& ascii,
                                          int& code, QEvent::Type &type, bool willRepeat)
{
  ...
  if (key < 128 || (key < 256 && (!input_mapper || input_mapper->mibEnum() == 4))) {
    ...
    if (qt_use_rtl_extensions && type == QEvent::KeyPress) {
      if (directionKeyEvent) {
        if (key == XK_Shift_L && directionKeyEvent == XK_Control_L || key == XK_Control_L && directionKeyEvent
            == XK_Shift_L) {
          directionKeyEvent = Key_Direction_L;
        } else if (key == XK_Shift_R && directionKeyEvent == XK_Control_R || key == XK_Control_R && directionKeyEvent
            == XK_Shift_R) {
...
static Bool qt_keypress_scanner(Display *, XEvent *event, XPointer arg)
{
  ...
  if (d->error || event->xkey.window != d->window || event->xkey.keycode != d->keycode)
    return FALSE;
...
static Bool qt_keyrelease_scanner(Display *, XEvent *event, XPointer arg)
{
  ...
  return (event->type == XKeyRelease && event->xkey.window == d->window && event->xkey.keycode == d->keycode);
}
...
bool QETWidget::translateKeyEvent(const XEvent *event, bool grab)
{
  ...
  translateKeyEventInternal(event, count, text, state, ascii, code, type, qt_mode_switch_remove_mask != 0);
  ...
  if (event->type == XKeyPress) {
    ...
    if (XCheckIfEvent(dpy, &nextpress, &qt_keypress_scanner, (XPointer) &auto_repeat_data)) {
      autor = TRUE;
  ...
  if (type == QEvent::KeyPress && !grab) {
    ...
    QKeyEvent a(type, code, ascii, state, text, autor, QMAX(QMAX(count, 1), int(text.length())));
  ...
  // do not compress keys if the key event we just got above matches
      ...
      !((code >= Key_Escape && code <= Key_SysReq) || (code >= Key_Home && code <= Key_Next) || (code >= Key_Super_L
          && code <= Key_Direction_R) || ((code == 0) && (ascii == '\n')))) {
    ...
    for (;;) {
      ...
      QEvent::Type t;
      translateKeyEventInternal(&evPress, countIntern, textIntern, stateIntern, asciiIntern, codeIntern, t);
  ...
  QKeyEvent e(type, code, ascii, state, text, autor, QMAX(QMAX(count, 1), int(text.length())));
...
static Bool isPaintOrScrollDoneEvent(Display *, XEvent *ev, XPointer a)
{
  ...
  if (ev->type == Expose || ev->type == GraphicsExpose || ev->type == ClientMessage && ev->xclient.message_type
      == qt_qt_scrolldone) {
...
void qt_insert_sip(QWidget* scrolled_widget, int dx, int dy)
{
  ...
  XSendEvent(appDpy, scrolled_widget->winId(), False, NoEventMask, (XEvent*) &client_message);
}
...
bool QETWidget::translatePaintEvent(const XEvent *event)
{
  ...
  QRect paintRect(event->xexpose.x, event->xexpose.y, event->xexpose.width, event->xexpose.height);
  ...
  if (merging_okay) {
    ...
    while (XCheckIfEvent(x11Display(), &xevent, isPaintOrScrollDoneEvent, (XPointer) &info)
        && !qt_x11EventFilter(&xevent) && !x11Event(&xevent)) // send event through filter
    {
      if (xevent.type == Expose || xevent.type == GraphicsExpose) {
        QRect exposure(xevent.xexpose.x, xevent.xexpose.y, xevent.xexpose.width, xevent.xexpose.height);
  ...
  //  erase(paintRegion);
  extern void aq_erase_region(QWidget*, const QRegion&);
  if (!isTopLevel()) {
    if (backgroundOrigin() != WidgetOrigin)
      erase(paintRegion);
    else
      aq_erase_region(this, paintRegion);
  }
...
bool QETWidget::translateConfigEvent(const XEvent *event)
{
  ...
  if (isTopLevel()) {    
    ...
    bool trust = (topData()->parentWinId == None || topData()->parentWinId == QPaintDevice::x11AppRootWindow());
    ...
    XEvent xevent;
    while (XCheckTypedWindowEvent(x11Display(), winId(), ConfigureNotify, &xevent) && !qt_x11EventFilter(&xevent)
        && !x11Event(&xevent))
      // send event through filter
      ;
  }
  ...
  // ### AbanQ
  if (transbg || (testWState(WState_Exposed) && was_resize) || QApplication::reverseLayout()) {

  // we ignore NorthWestGravity at the moment for reversed layout
  //if (transbg || (!testWFlags(WStaticContents) && testWState(WState_Exposed) && was_resize)
  //    || QApplication::reverseLayout()) {
    // remove unnecessary paint events from the queue
    XEvent xevent;
    while (XCheckTypedWindowEvent(x11Display(), winId(), Expose, &xevent) && !qt_x11EventFilter(&xevent)
        && !x11Event(&xevent))
      // send event through filter
      ;
...
static void sm_saveYourselfCallback(SmcConn smcConn, SmPointer clientData, int saveType, Bool shutdown,
  ...
  QObject(app, "session manager")
{
  ...
  smcConnection = SmcOpenConnection(0, 0, 1, 0, SmcSaveYourselfProcMask | SmcDieProcMask | SmcSaveCompleteProcMask
      | SmcShutdownCancelledProcMask, &cb, prevId, &myId, 256, cerror);
...
bool QSessionManager::allowsInteraction()
{
  ...
  if (sm_interactStyle == SmInteractStyleAny) {
    sm_waitingForInteraction
        = SmcInteractRequest(smcConnection, SmDialogNormal, sm_interactCallback, (SmPointer*) this);
  }
  if (sm_waitingForInteraction) {
    qApp->enter_loop();
    sm_waitingForInteraction = FALSE;
...
bool QSessionManager::allowsErrorInteraction()
{
  ...
  if (sm_interactStyle == SmInteractStyleAny || sm_interactStyle == SmInteractStyleErrors) {
    sm_waitingForInteraction = SmcInteractRequest(smcConnection, SmDialogError, sm_interactCallback, (SmPointer*) this);
  }
  if (sm_waitingForInteraction) {
    qApp->enter_loop();
    sm_waitingForInteraction = FALSE;
