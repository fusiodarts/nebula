...
extern void aq_update_frame(QWidget *);
...
QWidget::~QWidget()
{
  ...
  // ### AbanQ
  aq_notify_gui_event(this, new QEvent(QEvent::Destroy));
#ifdef AQ_WID_PM
  if (aq_pm_) {
    delete aq_pm_;
    aq_pm_ = 0;
...
bool QWidget::isEnabledTo(QWidget* ancestor) const
{
  ...
  while (w && !w->testWState(WState_ForceDisabled) && !w->isTopLevel() && w->parentWidget() && w->parentWidget()
      != ancestor)
    w = w->parentWidget();
...
void QWidget::setEnabled(bool enable)
{
  ...
  if (!isTopLevel() && parentWidget() && !parentWidget()->isEnabled() && enable)
...
void QWidget::windowActivationChange(bool)
{
  ...
  if (acg != icg) {
    ...
    if (bm > NoBackground && acg.brush(role) != icg.brush(role))
      setBackgroundFromMode();
    else if (acg.background() == icg.background() && acg.base() == icg.base() && acg.text() == icg.text()
        && acg.foreground() == icg.foreground() && acg.button() == icg.button() && acg.buttonText() == icg.buttonText()
        && acg.brightText() == icg.brightText() && acg.dark() == icg.dark() && acg.light() == icg.light() && acg.mid()
        == icg.mid() && acg.midlight() == icg.midlight() && acg.shadow() == icg.shadow())
      return;
    update();
  }
#endif
...
QRect QWidget::frameGeometry() const
{
  if (isTopLevel() && !isPopup()) {
    ...
    return QRect(crect.x() - top->fleft, crect.y() - top->ftop, crect.width() + top->fleft + top->fright,
...
{
  return extra ? QSize(extra->maxw, extra->maxh) : QSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX );
}
...
{
  return (extra && extra->topextra) ? QSize(extra->topextra->incw, extra->topextra->inch) : QSize(0, 0);
}
...
{
  return (extra != 0 && extra->topextra != 0) ? QSize(extra->topextra->basew, extra->topextra->baseh) : QSize(0, 0);
}
...
{
  if ( testWState(WState_OwnCursor) )
    return (extra && extra->curs) ? *extra->curs : arrowCursor;
  else
...
{
  return extra && extra->topextra ? extra->topextra->caption : QString::null;
}
...
{
  return (extra && extra->topextra) ? extra->topextra->iconText : QString::null;
}
...
void QWidget::setFocusProxy(QWidget * w)
{
  ...
  if (extra->focus_proxy) {
    disconnect(extra->focus_proxy, SIGNAL(destroyed()), this, SLOT(focusProxyDestroyed()) );
    extra->focus_proxy = 0;
  }

  if (w) {
    setFocusPolicy(w->focusPolicy());
    connect(w, SIGNAL(destroyed()), this, SLOT(focusProxyDestroyed()) );
  }
  extra->focus_proxy = w;
}
...
bool QWidget::focusNextPrevChild(bool next)
{
  ...
  do {
    if (w && w != startingPoint && ((w->focusPolicy() & focus_flag) == focus_flag) && !w->focusProxy()
...
bool QWidget::isActiveWindow() const
{
  ...
  if (style().styleHint(QStyle::SH_Widget_ShareActivation, this)) {
    if ((tlw->isDialog() || (tlw->testWFlags(Qt::WStyle_Tool) && !tlw->isPopup())) && !tlw->testWFlags(Qt::WShowModal)
        && (!tlw->parentWidget() || tlw->parentWidget()->isActiveWindow()))
      return TRUE;
    QWidget *w = qApp->activeWindow();
    if (!testWFlags(WSubWindow) && w && w->testWFlags(WSubWindow) && w->parentWidget()->topLevelWidget() == tlw)
      return TRUE;
    while (w && (tlw->isDialog() || tlw->testWFlags(Qt::WStyle_Tool)) && !w->testWFlags(Qt::WShowModal)
        && w->parentWidget()) {
...
QSize QWidget::frameSize() const
{
  if (isTopLevel() && !isPopup()) {
    ...
    QWidget *that = (QWidget *) this;
    QTLWExtra *top = that->topData();
    return QSize(crect.width() + top->fleft + top->fright, crect.height() + top->ftop + top->fbottom);
  }
  return crect.size();
}
...
{
  if (!widget || widget->isHidden() || widget->backgroundOrigin() == QWidget::WidgetOrigin
...
{
  // ### AbanQ
  if (aq_frame_dirty_ || (fstrut_dirty && isTopLevel() && !isPopup()))
    aq_update_frame(this);

  QPoint oldp(pos());
  internalSetGeometry(x + geometry().x() - QWidget::x(), y + geometry().y() - QWidget::y(), width(), height(), TRUE );
...
void QWidget::show()
{
  ...
  if (extra) {
    int w = crect.width();
    int h = crect.height();
    if (w < extra->minw || h < extra->minh || w > extra->maxw || h > extra->maxh) {
...
bool QWidget::close(bool alsoDelete)
{
  ...
  if (!deleted && !isHidden())
    hide();
  if (checkLastWindowClosed && qApp->receivers(SIGNAL(lastWindowClosed()))) {
    ...
    while (widget) {
      if (!widget->isHidden() && !widget->isDesktop() && !widget->isPopup() && (!widget->isDialog()
          || !widget->parentWidget()))
        break;
      widget = list->next();
    }
...
bool QWidget::isVisibleTo(QWidget* ancestor) const
{
  if (!ancestor)
    return isVisible();
  const QWidget * w = this;
  while (w && w->isShown() && !w->isTopLevel() && w->parentWidget() && w->parentWidget() != ancestor)
    w = w->parentWidget();
  return w->isShown();
}
    ...
    case QEvent::WindowDeactivate:
      ...
      if (children()) {
        ...
        while ((o = it.current()) != 0) {
          ++it;
          if (o->isWidgetType() && ((QWidget*) o)->isVisible() && !((QWidget*) o)->isTopLevel())
...
void QWidget::reparent(QWidget *parent, WFlags f, const QPoint &p, bool showIt)
{
