...
public:
  ...
  void drawImage(int x, int y, const QImage &, int sx = 0, int sy = 0, int sw = -1, int sh = -1, int conversionFlags =
      0);
  ...
  void drawText(int x, int y, int w, int h, int flags, const QString&, int len = -1, QRect *br = 0,
                QTextParag **intern = 0);
  ...
  void drawTextItem(int x, int y, const QTextItem &ti, int textflags = 0, const QRect &brect = QRect());
...
protected:
  ...
  friend void qt_format_text(const QFont &, const QRect &r, int tf, const QString& str, int len, QRect *brect,
                             int tabstops, int* tabarray, int tabarraylen, QTextParag **internal, QPainter* painter);
...
private:
  ...
  // ### AbanQ
  friend void aq_bitblt(QWidget*, int, int, const QPaintDevice*, int, int, int, int);
};
...
inline void QPainter::drawImage(const QPoint &p, const QImage &pm, const QRect &sr, int conversionFlags)
{
  drawImage(p.x(), p.y(), pm, sr.x(), sr.y(), sr.width(), sr.height(), conversionFlags);
}
...
inline void QPainter::drawText(int x, int y, int w, int h, int tf, const QString& str, int len, QRect *br,
                               QTextParag **i)
{
  QRect r(x, y, w, h);
...
inline QRect QPainter::boundingRect(int x, int y, int w, int h, int tf, const QString& str, int len, QTextParag **i)
{
  QRect r(x, y, w, h);
