...
public:
  ...
  QEvent(Type type) : t(type), posted(FALSE), spont(FALSE), aq_ignore(FALSE) {}
  virtual ~QEvent();
  Type  type() const  {
    return t;
  }
...
public:
  ...
  QPair<int, int> uniqueId() {
    return QPair<int, int>(mType, mPhy);
  }
protected:
  QPoint mPos;
  QPoint mGPos;
...
public:
  ...
  bool inserted() const {
    return t == ChildInserted;
  }
  bool removed() const {
    return t == ChildRemoved;
  }
protected:
  QObject *c;
};
